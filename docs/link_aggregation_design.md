...
are:
* "forwarding": At least one of the member interfaces is in a forwarding state
* "blocked": All member interfaces are blocked by LACP (not in the collecting /
  ...
  blocked.
* "tx_disable": All member interfaces are in a "tx_disable" state
...
* "forwarding": Indicates that the interface is up and forwarding traffic.
* "blocked": Indicates that the interface is not forwarding traffic in neither
rx or tx direction
...
* "tx_disable": The LACP definition includes an interface state where it can be
collecting (receiving) traffic but not sending it.  This key is used to Indicate
that state where the interface can receive but will not sent traffic out of the
LAG
