...
supported by every switch chip that implements link aggregation. The _advanced_ mode is supported based on the chip capabilities . The default value for `bond_mode` is _"l3-src-dst-hash"_.
...
`hw_bond_config` to determine if an interface is eligible to be a member ot a
