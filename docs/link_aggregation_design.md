...
The `lacp_fallback_timeout` key is used determine the time during which fallback
will be active.  The timer starts counting when the interface's state machine
gets to the defaulted state.  When the timer expires the interfaces will get
blocked as if fallback were disabled.
