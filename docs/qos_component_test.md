...
4. **Port Trust Override**
    - Validate 'show running-config'
        - Shows the port trust override, if it is set to the factory-default value.
        - Shows the port trust override, if it is not set to the factory-default value.
...
6. **Queue Profile**
    ...
    - Validate 'qos queue-profile p1' fails if profile p1 is applied
    - Validate 'no qos queue-profile' fails with illegal or missing name
        - Illegal length
        - Unexpected characters
    - Validate that 'no qos queue-profile strict' fails
    - Validate 'no qos queue-profile p1' fails if profile p1 is applied
...
7. **Schedule Profile**
    ...
    - Validate 'qos schedule-profile p1' fails if profile p1 is applied
    - Validate 'no qos schedule-profile' fails with illegal or missing name
        - Illegal length
        - Unexpected characters
    - Validate that 'no qos schedule-profile strict' fails
    - Validate 'no qos schedule-profile p1' fails if profile p1 is applied
...
8. **Global Apply Queue Profile and Schedule Profile**
    ...
    - Validate 'apply qos' fails if the queue profile has fewer queues than the schedule profile
    - Validate 'apply qos' fails if the queue profile has more queues than the schedule profile
...
9. **Port Schedule Profile Override**
    - Validate port override 'apply qos' succeeds with valid queue profile and schedule profile
    - Validate port override 'apply qos' fails if the queue profile has fewer queues than the schedule profile
    - Validate port override 'apply qos' fails if the queue profile has more queues than the schedule profile
    ...
    - Validate 'no apply qos schedule-profile' removes the port dscp override
...
1. **Port Table**
    ...
    - Validate Port PUT fails if port cos override does not have port trust mode 'none'
    - Validate Port PUT fails if port dscp override does not have port trust mode 'none'
    - Validate Port PUT fails if the port queue profile override is not null
        - This is because port queue profile override is not currently supported
    - Validate Port PUT fails if the port schedule profile override does not have the same algorithm on all queues
    - Validate Port PUT fails if the port schedule profile override does not contain the same queues as the globally applied queue profile

2. **Q_Profile_Entry Table**
    - Validate Q_Profile_Entry PUT succeeds with valid config
    - Validate Q_Profile_Entry PUT fails if the entry is part of an applied profile
    - Validate Q_Profile_Entry PUT fails if the entry is part of the factory-default profile
    - Validate Q_Profile_Entry PUT fails if the name contains illegal characters
    - Validate Q_Profile_Entry DELETE succeeds with valid config
    - Validate Q_Profile_Entry DELETE fails if the entry is part of an applied profile
    - Validate Q_Profile_Entry DELETE fails if the entry is part of the factory-default profile

3. **Q_Profile Table**
    - Validate Q_Profile PUT succeeds with valid config
    - Validate Q_Profile PUT fails if the profile is an applied profile
    - Validate Q_Profile PUT fails if the profile is the factory-default profile
    - Validate Q_Profile POST succeeds with valid config
    - Validate Q_Profile POST fails if the name contains illegal characters
    - Validate Q_Profile POST fails if the name is 'strict'
    - Validate Q_Profile DELETE succeeds with valid config
    - Validate Q_Profile DELETE fails if the profile is an applied profile
    - Validate Q_Profile DELETE fails if the profile is the factory-default profile
    - Validate Q_Profile DELETE fails if the profile is the default profile

4. **QoS_COS_Map_Entry Table**
    - Validate QoS_COS_Map_Entry PUT succeeds with valid config
    - Validate QoS_COS_Map_Entry PUT fails if the description contains illegal characters
...
5. **QoS_DSCP_Map_Entry Table**
    ...
    - Validate QoS_DSCP_Map_Entry PUT fails if the description contains illegal characters
...
6. **QoS Table**
    ...
    - Validate QoS PUT fails if the profile is an applied profile
    - Validate QoS PUT fails if the profile is the factory-default profile
    - Validate QoS POST succeeds with valid config
    - Validate QoS POST fails if the name contains illegal characters
    - Validate QoS POST fails if the name is 'strict'
    - Validate QoS DELETE succeeds with valid config
    - Validate QoS DELETE fails if the profile is an applied profile
    - Validate QoS DELETE fails if the profile is the factory-default profile
    - Validate QoS DELETE fails if the profile is the default profile

7. **Queue Table**
    - Validate Queue PUT succeeds with valid config
    - Validate Queue PUT fails if the entry is part of an applied profile
    - Validate Queue PUT fails if the entry is part of the factory-default profile
    - Validate Queue PUT fails if the algorithm is wrr and the weight is greater than the max weight
    - Validate Queue DELETE succeeds with valid config
    - Validate Queue DELETE fails if the entry is part of an applied profile
    - Validate Queue DELETE fails if the entry is part of the factory-default profile

8. **System Table**
    - Validate System PUT succeeds with valid config
    - Validate System PUT fails if global qos_trust is empty
    - Validate System PUT fails if the global queue profile does not have all local priorities
    - Validate System PUT fails if the global queue profile contains duplicate local priorities
    - Validate System PUT fails if the global schedule profile does not have the same algorithm on all queues
    - Validate System PUT fails if the global queue profile and the global schedule profile do not contain the same queues
    - Validate System PUT fails if the global queue profile and any port schedule profile do not contain the same queues
