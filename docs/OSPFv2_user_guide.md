...
A sample topology and configuration example follows:
...
switch(conf)#network 20.20.20.0/24 area 1
```
Using the above configuration OSPFv2 can be enabled on interface 1 of Switch 1, interface 1 and 2 of Switch 2 and interface 1 of switch 3. And the adjacency will be formed between them. Switch 2 will act as an ABR.

### OSPFv2 supportability
VLOG and ZLOG are the logging which can be used for troubleshooting purpose. Logging levels for VLOG and ZLOG can be changed through following CLI's :
...
This will enable the specified logging level to be configured and only those logs will appear that have priority below than the specifed level in CLI.


#### Enabling the debug knobs for OSPFv2
The following CLI can be used for enabling the debug nobs in OSPFv2 daemon for troubleshooting purpose :

For example:

    switch(conf)# debug ospfv2 packet (hello|dd|ls-request|ls-update|ls-ack|all)

The above CLI will enable the "packet" knobs and the logging destination will contain the logs for packet sent between routers.
...
##### Cause
- If the priority is zero on all the OSPFv2 routers, then none of the routers are selected as DR or BDR. The `show ip ospf neighbor` command lists the neighbors and their priorities.
