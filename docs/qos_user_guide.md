...
- [Configuring QoS Trust](#configuring-qos-trust)
    ...
    - [Configuring 802.1D Ethernet class of service](#configuring-802-1d-ethernet-class-of-service)
...
## Overview
Quality of Service (QoS) features give network devices the ability to customize servicing behaviors to different kinds of traffic reflecting each traffic type's unique characteristics and importance your organization:
...
Deficit Weighted Round Robin can provide a fairer distribution of available bandwidth across the priorities that limits queue starvation.  Lower priority queues will have some service even when packets are present in higher priority queues. The degree this occurs depends on the weights assigned to each queue.
...
At the start of the round, all non-empty queues' deficit counter are incremented by a quantum value in proportion to their weight.  Then the scheduler will inspect the non-empty queues in decreasing priority order comparing the queue's deficit counter against the packet size at the head of the queue.  The round ends when a deficit counter is found that is larger than the packet size.  That packet is de-queued for transmission and the queue's deficit counter is decremented.

In general, lower priority queues deficit counters will be larger (i.e. they build up a deficit) than higher priority queues deficit counters.  With a mix of large and small packet sizes of high priority  traffic, higher priority queues deficit counters will have some rounds where they will be smaller than the size of the queue at head of queue.  This occasionally allows traffic from lower priority queues to be selected for transmission.
...
Using strict priority for all queues would be good for the voice but at the risk of lower priority queue starvation.  DWRR for all queues would be fairer to the lower priority traffic but risk increased voice traffic delay and jitter.
...
**Note:** The only way to remove qos trust mode from the running configuration display is to revert back to the initial trust mode via **no qos trust**. The same for port overrides, only **no qos trust** will remove the override from the running configuration display.
...
```
or configure only one Ethernet port or all the members of a Link Aggregation Group (LAG):
```ditaa
    #configure terminal
    (config)# interface 1
...
In **qos trust cos** is configure the network device uses the **Cos Map** to determine which local-priority and color to assign the packet.  To display the configuration of the Cos Map:
```ditaa
    # show qos cos-map
...
IEEE 802.1Q is the most current Ethernet standard for class of service.  It superseded an earlier standard 802.1D in 2005.  IEEE 802.1Q slightly changed the ordering the classes of service from its predecessor IEEE 802.1D for CoS 2 and Cos 0:
...
Notice in 802.1D both Cos 2 and CoS 1 are below CoS 0 (Best Effort).

When an OpenSwitch network device is installed in a network of devices following 802.1D class of service, the QoS Cos Map must be re-configured to follow the 802.1D standard by swaping the assignments of CoS 0 and 2:
```ditaa
    #configure terminal
...
For **qos trust dscp** the network device uses the **DSCP Map** to determine which local-priority and color to assign the packet.  Use **show qos dscp-map** display the configuration of the DSCP Map.
...
Finally, use **apply qos queue-profile NAME** command to configure all ports to use the new profile.
...
Many organizations network carries voice over IP (VoIP) traffic.  It is delay and jitter sensitive, so dwell time in network devices must be kept at a minimum. It is critical that all network devices in the path have their per-hop behaviors configured identically to handle this traffic.
...
As a prerequisite, voice traffic packets must be uniquely identified.  Many networks using DiffServe mark voice packets with Expedited Forwarding (EF) DSCP.

To configure all ports to have DSCP EF packets use the highest priority queue that is serviced strictly first before any other queues' packets, follow these five steps:
1. Change default DSCP Map code point to local-priority assignments so EF has its own local-priority (5)
2. Configure a queue profile that puts the EF packets in the highest priority queue
3. Configure a SP plus DWRR schedule profile
4. Apply the queue and schedule profiles
5. Configure global trust mode to DSCP

**Step 1: Change default DSCP Map for code points using local-priority 5**
The default DSCP Map has DSCP EF assigned to local-priority 5.  It is necessary to have packets with DSCP EF be the sole user of a local priority 5.  The default DSCP Map has 8 code points, 40 through 47 (CS5) mapping to local-priority 5.  The other seven code points have to be reassigned to another local-priority depending what protocol(s) are using these code points in your network.  This example assumes CS5 is used by Call Signaling protocols and will assigned local-priority 6.
...
```
**Step 3: Create a SP plus DWRR schedule profile**
Create a schedule profile that services queue 7 using strict priority (SP) and the remaining queues using deficit weighted round robin (DWRR).  This example will give all DWRR queues equal weight.  The actual weight values will be different in your network.
