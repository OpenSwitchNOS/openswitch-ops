...
* A new JSON RPC method (`process_identify`), that the client uses to identify
...
* The ability to call the `process_identify` command from the ovsdb-client
  (mostly for testing purposes).

### Changes to JSON RPC

The method `process_identify` was introduced. It has a single parameter the
