...
### Background
Different clients has different response time requirements from the OVSDB. A
program used by a user could wait a little more than a daemon critical for the
correct operation of the switch.

With *priority sessions* is possible to identify the clients within the OVSDB
Server, and the server can give more priority to the process that need it the
most.

This feature is composed of:

* A new JSON RPC method (identify), that the client uses to identify the
...
* The ability to call the identify command from the ovsdb-client (mostly for
testing purposes).

### Changes to JSON RPC

The method `identify` was introduced. It has as single parameter the name or
identifier of the session. The response is a integer between 0 and 15
...
{
    "method": "identify",
    "params": ["<name or identifier of the session"],
    "id": ...
}
...
connections. With this feature the OVSDB Server respond only to the sessions
...
executed in 14 of each 16 iterations and so on. Therefore, the sessions are run
in at least the following percentage of iterations:
...
To avoid iterations in which the OVSDB Server "do nothing" it process the
...
Instead of allowing the client to request a priority, this system works with an
...
In case that a identifier wasn't specified in this file then the OVSDB Server
assigns it the default priority (8). The file can be reloaded in runtime, using
