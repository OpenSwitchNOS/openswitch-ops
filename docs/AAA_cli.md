...
```
#### Description ####
This command configures the number of retries to the RADIUS server host from the switch. The authentication takes place accordingly.
...
This command is used to configure the timeout period for the RADIUS server host on the switch.
#### Syntax ####
```
[no] radius-server timeout <1-60>
```
#### Description ####
This command configures the timeout in seconds to the RADIUS server host from the switch. The authentication takes place accordingly.
