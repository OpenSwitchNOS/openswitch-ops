...
interface or port state.  __Figure 1__ shows the main layers for interfaces and
ports.  The priority is higher on the lowest layers, if a layer is down or
blocked then the layers above it are irrelevant.  In the figure the
