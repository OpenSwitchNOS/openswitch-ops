...
When vtysh is started the user has already been authenticated. Vtysh will need to make a call to RBAC to either check specific permissions or get the list of permissions accessible to the user. Vtysh will need to restrict the user from accessing information that is not allowed under their permission list depending on which role (ops_admin, ops_netop, none) the authenticated user has been assigned.

Bash access (start-shell) will no longer be allowed for netop users under vtysh. In addition, vtysh should restrict the use of system calls made by using seccomp or some equivalent functionality.

Vtysh is the shell that netop users should be launched into. Admin user will be launched into the bash shell, however they will be able to run vtysh if they desire.
...
Ansible will connect to the switch via SSH (key or user/pass) to do system configuration management and provisioning. It is currently being investigated what role the ansible user should belong to and we will need to scope the user's account accordingly.
...
We will be supporting 2 pre-defined roles. The table below briefly describes the permissions for each role. Any user created that is not a member of one of the two pre-defined roles will have the role of "none" and will be restricted from accessing content in the the switch management interface (CLI, REST, WebUI) .
...
Currently, OpenSwitch's CLI and REST management interfaces adds a user by making a call to "useradd" and removes a user by calling "userdel". If these mgmt interfaces plan on supporting SYS_MGMT operations (admin operations) then they could support user add/del operations but they will need to support roles when creating the users. Outside of the mgmt interfaces, user creation/deletion will be handled in the bash shell by the system admin. To support the two roles, we will be using unix groups (as shown below)
...
* netop - Will be a member of both the ops_netop and ovsdb-client group and will have default vtysh shell.
