...
## Statistics
Layer3 statistics are supported for physical and virtual interfaces (VLAN and subinterfaces) which have layer3 enabled. OpenSwitch supports counters for RX or TX packets and bytes, for layer3 unicast and multicast traffic. These counters only count routed traffic and do not account for switched L2 traffic, that is traffic within a given broadcast domain or VLAN. On physical L3 interfaces, the L3 counters are also supported at a protocol specific (IPv4 or IPv6) level. There is no support for IPv4 or IPv6 specific statistics for VLAN interfaces, subinterfaces and loopback interfaces.
