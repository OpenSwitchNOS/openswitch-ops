...
If the platform supports it, OpenSwitch can enable "resilient ECMP" to preserve in-flight traffic flows when ECMP group membership changes.
To enable resilient ECMP, use the following commands:
```
ops-as5712# configure terminal
ops-as5712(config)# ip ecmp load-balance resilient
```
To disable resilient ECMP:
```
ops-as5712# configure terminal
ops-as5712(config)# no ip ecmp load-balance resilient
```
