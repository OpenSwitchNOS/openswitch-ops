...
## Statistics
Layer3 statistics are supported for physical and virtual interfaces (VLAN and Sub-interfaces) which have layer3 enabled. OpenSwitch supports counters for RX/TX packets and bytes, for layer3 unicast and multicast traffic. On physical L3 interfaces, the same counters are also supported at a protocol specific (IPv4/IPv6) level. There is no support for protocol specific statistics on virtual L3 interfaces. Currently, OpenSwitch accounts for all ingress L3 traffic and traffic that is forwarded directly through the ASIC. Egress traffic that is either originated from the switch or that was forwarded by the linux kernel via slow-path is not accounted for.

The following commands are used to view L3 statistics:

Show L3 statistics for a interface 1.
```
...
Interface 1 is up
 ...
 TX
          ...
          mcast: 0 packets, 0 bytes
```
Show IPv6 specific L3 statistics for interface 1 (not supported for virutal L3 interfaces).
```
