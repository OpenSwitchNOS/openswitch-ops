...
#### Syntax
`sflow agent-interface <interface-name> [agent-address-family <ipv4/ipv6>]`
#### Description
This command sets the name of the interface whose IP address is used as the agent IP in the sFlow datagrams.
If not specified, the system picks the IP address from one of the interfaces in a priority order (to be determined). It also sets the family type(IPv4 or IPv6) for te agent interface.
#### Authority
All users.
#### Parameters
| Parameter | Status   | Syntax         | Description                           |
...
```
ops-as5712# configure terminal
ops-as5712(config)# sflow agent-interface 1 agent-address-family ipv4
```
