...
#### Description
Enables support for the DHCP relay feature on all interfaces. A DHCP relay agent receives DHCP requests from clients and forwards them to one or more DHCP servers. The DHCP server(s) to which the requests are forwarded must be configured individually on each interface using the `ip help-address` command.
...
It is not necessary for all relay agents on the path between a DHCP client and the server to support Option 82. A relay agent without Option 82 forwards DHCP packets regardless of whether they include Option 82 fields. However, Option 82 relay agents should be positioned at the DHCP policy boundaries in a network to provide maximum support and security for the IP addressing policies configured in the server.
...
| **keep**    | Required | Literal | 	If the relay agent receives a client request that already has one or more Option 82 fields, the keep parameter causes the relay agent to retain such fields and forward the request without adding another Option 82 field. If the incoming client request does not already have any Option 82 fields, the relay agent appends an Option 82 field before forwarding the request.|
| **drop**    | Required | Literal | Drops an inbound client requests with an Option 82 field already appended. If no Option 82 fields are present, the drop parameter causes the switch to add an Option 82 field and forward the request. As a general guideline, configure the drop parameter on relay agents at the edge of a network, where an inbound client request with an appended Option 82 field may be unauthorized, a security risk, or for some other reason should not be allowed.|
...
#### Description
Configures a gateway address that the DHCP relay agent uses for DHCP requests. If not configured, the DHCP relay agent automatically uses the lowest-numbered IP address. Only supported for IPv4.
