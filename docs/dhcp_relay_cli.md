...
#### Description
This command supports multiple helper-address configurations under an interface context, and is used to configure/unconfigure a remote DHCP server IP address on the device. Here the helper address is the same as the DHCP server address. A maximum of 16 helper-addresses per interface can be configured.
Even if routing is disabled on an interface, helper address configuration is allowed, but interface DHCP relay functionality will be inactive. In case a client has received an IP address, and no routing is configured, the IP address is valid on the client until the lease time expires.
#### Authority
All users.
#### Parameters
