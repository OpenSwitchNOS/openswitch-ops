...
```
The configuration of VLAN involves VLAN ID, which is used by switches to determine which port(s) or interface(s) to send broadcast packet to. Based on this knowledge, this vlan role will involve schema for both VLAN and Port.
...
```
Refer to [schema/vswitch.extschema](http://git.openswitch.net/cgit/openswitch/ops/tree/schema/vswitch.extschema), the two top level tables we need are <i>System</i> table, which contains the column <i>bridges</i>, and <i>Port</i> table.
```
...
  "tables": {
    "System": {
...
```
Refer to <i>Ports</i> table of [schema/vswitch.extschema](http://git.openswitch.net/cgit/openswitch/ops/tree/schema/vswitch.extschema), create the ops_port.j2 template(<b>templates/ops_system_bridges.j2</b>) :
```
    ...
    - role: vlan
      ...
      ops_ports:
        ...
        - name: 3
          tag: 3
```
To fully explore the beauty of Ansible, add multiple switch members to your <b>hosts</b> file and group them together, and they will be configured simultaneously when execute the playbook (e.g. <b>tests/create_vlan.yml</b>). The following is a <b>hosts</b> file example:
...
**Note:** For more ansible roles example for openswitch, check [ops-ansible/roles](http://git.openswitch.net/cgit/openswitch/ops-ansible/tree/roles)
