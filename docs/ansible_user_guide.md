...
With basic Ansible knowledge from previous sections. Now we are ready to create our first Ansible role for OpenSwitch. This instruction will demonstrate how to create an Ansible role for OpenSwitch VLAN configuration and testing.

### Create and initialize VLAN role
To begin with, we will first initialize vlan role under <b>/etc/ansible/roles</b> folder with ansible-galaxy API
```
ansible-galaxy init vlan
```
Create default file for the vlan role as <b>defaults/main.yml</b>
```
---
# defaults/main.yml
...
ops_debug: false
```
Create task file to configure vlan and debug configuration transaction as <b>tasks/main.yml</b>
```
---
...
- name: result from the switch
  ...
  when: ops_debug
```
Now we are ready to create templates with the help from schema.

### Create templates based on schema
To begin with, we first create <b>templates/main.j2</b> file
```
...
```
Refer to <b>schema/vswitch.ovsschema</b> in ops repository, we can see the main table for ops is named <i>System</i>, and it have column <i>bridges</i> that we need
```
...
  "tables": {
    "System": {
  ...
  ...
```
So we create a template to iterate through <i>System</i> table as <b>templates/ops_system.j2</b>
```
...
```
Notice in the schema sample, <i>bridges</i> is referred to <i>Bridges</i> table.
```
"refTable": "Bridge"
```
And in <i>Bridge</i> table, there is a <i>vlan</i> column that refers to <i>VLAN</i> table
```
"vlans": {
          "type": {
            "key": {
...
```
Based on this fact, we create template to iterate through <i>bridges</i> column as <b>templates/ops_system_bridges.j2</b>
```
...
```
Finally, we can create vlan template <b>templates/ops_system_vlans.j2</b>
```
...
Here is a simple playbook to create two vlans under default bridge (<b>tests/create_vlan.yml</b>)
