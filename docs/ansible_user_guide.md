...
- [Intalling Ansible](#installing-ansible)
	- [Setting up the basic configuration on a control machine](#setting-up-the-basic-configuration-on-a-control-machine)
		...
		- [Declaring an inventory/hosts file](#declaring-an-inventory/hosts-file)
...
```
Verify the VLAN configuration on the OpenSwitch host by `show vlan` command:
```
switch# show vlan
...
```
For more information, refer to the [Declaring an inventory/hosts file](#declaring-an-inventory/hosts-file) session.
