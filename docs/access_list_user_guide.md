...
- [How to use the feature](#how-to-use-the-feature)
    ...
    - [Displaying hardware resource suage](displaying-hardware-resource-usage)
...
all IP traffic that does not match any sequentially higher entries in the list.
...
1. Traffic expected to be permitted and is blockde
...
ACL consume resources from a limited hardware pool.  An ACL will not consume
hardware resources until it is applied to an interface.  Many more ACLs may be
configured than can be applied in hardware.  To display the consumption of the
hardware resources use this command:

    switch# diag-dump hw-resource basic

Depending on the hardware implemantion for the prodcut, the command will display
