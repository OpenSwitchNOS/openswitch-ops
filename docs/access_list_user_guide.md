...
lower) entry in the list.  An ACL with no user configured entries that is
applied to an interface, is programmed in hardware with the implicit deny entry.
...
than can be applied in hardware. Use the followinng command to display the
