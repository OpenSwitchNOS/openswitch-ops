...
- [How to use the feature](#how-to-use-the-feature)
    ...
    - [Troubleshooting the configuration](#troubleshooting-the-configuration)
         ...
         - [Condition: An empty ACL is applied to an interface and all IP traffic is blocked](#contdition:-an-empty-acl-is-applied-to-an-interface-and-all-ip-traffic-is-blocked)
...
in the list.  This entry is known as the implicit deny entry and will apply to
all IP traffic that does not match any sequentially higher (numerically lower)
entry in the list.  An ACL with no user configured entries that is applied to an
interface will be programmed in hardware with the implicit deny entry.
...
If an ACL is deleted while applied to any interfaces, the ACL will automatically
be unapplied from all interfaces.
...
have been configured and accepted by the system.  In the case of applied ACLs,
the active configuration will display the interfaces on which the ACLs have
successfully been programmed in hardware.
...
active on the product.  Unsupported command parameters may have been configured,
...
there is a discrepancy between what has been configured and what is active, run
either the command `show access-list commands` or
`show access-list commands configuration`.  If the active ACLs and configured
ACLs are not the same a warning message will be displayed.
...
### ACL Logging
ACL logging is a useful tool for troubleshooting ACLs.  It can be used to
identify traffic in the following scenarios:

1. Traffic expected to be permitted and is blocked
2. Unexpected traffic in the network

When the `log` keyword is specified for an ACE, packets that match this entry
will be copied to the switch CPU for processing by the ACL logging feature.  The
first packet that matches any log enabled ACE in an ACL will be logged to the
configured system log.  The reception of this packet will also start the ACL
logging timer.  Subsequent packets that match log enabled ACEs will not be
logged to the system log for the duration of the ACL logging timer.  When the
ACL logging timer expires a list of all the ACLs with logging enabled ACEs with
their corresponding hitcounts will be logged to the system log.  The next packet
that matches a log-enabled ACE will be logged, repeating the process of starting
the ACL logging timer.  The ACL logging feature displays the ACL hitcounts
stored in the database which are read from hardware every 5 seconds.
...
#### Display logging messages via the vtysh shell:
...
ACLs consume resources from a limited hardware pool.  An ACL will not consume
hardware resources until it is applied to an interface.  Many more ACLs may be
created than can be applied in hardware.  To display the consumption of the
hardware resources use this command:
