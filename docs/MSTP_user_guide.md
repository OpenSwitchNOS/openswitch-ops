...
|Events                                                                |Description |
|----------------------------------------------------------------------|------------|
|MSTP Enabled                                                          |MSTP feature enabled in the device.|
|MSTP Disabled                                                         |MSTP feature disabled in the device|
|{config_parameter} should be {value}                                  |Bad configuration parameter|
|BPDU has {config_parameter} from port {value}                         |Bad configuration in the received bpdu from the port|
|Bad reconfiguration request: {reconfig_parameter}                     |Bad reconfig parameter|
|Root changed from {old_priority}:{old_mac} to {new_priority}:{new_mac}|MSTP root has changed|
|Port {port} disabled {sec}- BPDU received on protected port           |MSTP BPDU received on protected port|
|{proto} starved for {pkt_type} on port {port} from {priority_mac}     |MSTP BPDU not received on port with in hello interval
|BPDU loss- port {port} moved to inconsistent state for {proto}        |MSTP is in inconsistent state|
|Topology Change received on port {port} for {proto} from source: {mac}|MSTP topology change received from port|
|{proto} - Topology Change generated on port {port} going in to {state}|MSTP topology change generated on port|
|BPDU received on admin edge port {port}                               |MSTP BPDU received on admin edge port |
|Port {port} blocked on CIST                                           |Port blocked for CIST|
|Port {port} unblocked on CIST                                         |Port unblocked for CIST|
|Port {port} blocked on MST{instance}                                  |Port blocked for MSTI|
|Port {port} unblocked on MST{instance}                                |Port unblocked for MSTI|
|{proto} Root Port changed from {old_port} to {new_port}               |Root port chnaged|
