...
The POST request for an event subscription results in the URI of the new resource in the response, which includes the index of the event relative to the subscriber. For example, the first event subscription for subscriber `ws_1` will result in a response with the `/rest/v1/system/event_subscribers/ws_1/event_subscriptions/0` URI. The `uri` field will be validated and will result in a `400` error response, such as `Invalid resource URI` if the resource does not exist. Upon a successful subscription, the initial values for the subscribed URIs are sent as notifications to the client.
