...
A second IDL is used to monitor for specific changes to increase the performance by avoiding the need to use the existing REST daemon's IDL which subscribes to every resource and attributes. The second IDL is used only for event subscriptions, and is reconfigured if a subscription doesn't already exist. This helps to avoid false change notifications from the DB, and prevent unnecessary event checking.
