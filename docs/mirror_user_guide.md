...
As mentioned previously, a source Interface can be mirrored in one of 3 different forms.  Inbound only (rx), outbound only (tx) or both inbound & outbound (both).

If no direction is specified, the specified Interface is completely removed as a source in all directions regardless of the direction it was originally added, including 'both'.
```ditaa
(config)# mirror session mirror_3
(config-mirror)# no source interface 2
```
This is the simplest way to remove a source Interface entirely.
To remove a source Interface added with 'both', also use the directionless form. 'both' is not a valid option for removal.

A direction may be specified to remove mirroring in that particular direction if already added.  If the switch is not mirroring with the source Interface in the direction specified, whether explicitly or via 'both', there is no change.
```ditaa
(config)# mirror session mirror_3
(config-mirror)# no source interface 2 rx
```
Regarding an existing 'both' source Interface, and direction specific removal:
If a source Interface was originally added with 'both', removing direction 'rx' will remove mirroring from just that direction but leave the mirroring active in the 'tx' direction.  Conversely, removing direction 'tx' will leave mirroring active in the 'rx' direction.
In other words, for a 'both' source Interface, you can later restrict mirroring from that source Interface in a particular direction by removing mirroring in the opposite direction.


As an alternative to source Interface modification via the 'no' form of the 'source interface' command, you can re-specify an Interface's source mirroring preferences which will replace that Interface's current configuration.
