...
parent_idl = validation_args.p_resource_idl
...
child_idl = validation_args.resource_idl
...
The `p_resource_idl` and `resource_idl` arguments are resource specific `Idl` objects from the `ovs.db.idl` module. The IDL includes the resource's specific table, rows, and columns information. For example, the following code snippet obtains the rows from a table:
...
for row in parent_idl.rows.itervalues():
