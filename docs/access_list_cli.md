...
Creates an Access List (ACL) comprised of one or more Access Control
...
An ACL will continue to process a packet until either the last ACE in the list
has been evaluated or the packet matches an ACE.

Note that an ACL must be applied via the `apply` command (at interface context)
before it will have an effect on traffic.
