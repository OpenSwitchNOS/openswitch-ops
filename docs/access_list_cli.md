...
Reset the entry configuration for the named ACL (or all ACLS) to match the
active configuration. Active configuration means the configuration has passed
platform support and capacity checks and is either programmed or pending
...
Reset the IPv4 ACL "My_ACL" entry configuration.

```
switch(config)# reset access-list ip My_ACL
```

Reset the entry configuration for all ACLs.
...
```
reset access-list applied {all | [{interface|vlan} <id>] [in] [ip]}
```

#### Description

Reset the apply configuration for one or more ACLs to match the active
...
|---------------|----------|---------|-------------|
| *all*         | Required | Keyword | Operate on all ACLs. |
| *interface*   | Required | Keyword | Operate on a specified interface name. |
| *vlan*        | Required | Keyword | Operate on a specified VLAN ID. |
| *id*          | Required | String  | The name or ID of the interface or VLAN. |
| **in**        | Optional | String  | Operate only on ingress ACLs. |
| **ip**        | Optional | Keyword | Operate only on IPv4 ACLs. |

#### Examples

Reset the apply configuration of ACLs on interface 1.

```
switch(config)# reset access-list apply interface 1
```

Reset the apply configuration of all ACLs.

```
switch(config)# reset access-list apply all
```
