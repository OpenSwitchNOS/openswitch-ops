...
no domainname
```
Command "domainname <name>" is used to configure the domainname of the system through CLI.Command "no domainname" removes the configured system domainname.
#### Authority
Admin
#### Parameters
...
```
  ...
  abc(config)# domainname example.com
  abc.example.com(config)#no domainname
  abc(config)#
```
