...
The OpenVswitch (OVS) module is built in OpenSwitch (OPS) by downloading its
...
the [openswitch/ops-build repository](
...
`feature/branch-2.5`. ovs has special branches associated with each release that
follow the name convention: `patches/branch-<OVS_VERSION>`. It contains all the
...
Copy the new patch to
`yocto/openswitch/meta-distro-openswitch/recipes-ops/openvswitch/ops-openvswitch`
and include it in the recipe by editing the file `ops-openvswitch.bb`.
...
it does not.
```
// if it already exists, then move to it and update it
git checkout --track -b feature/<FEATURE_NAME>
git rebase patches/branch-<OVS_VERSION>
...
configured to support the workflow described in this section.
