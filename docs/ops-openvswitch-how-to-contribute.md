...
This approach keeps unchanged in openvswitch/ovs master branch, so that the
repository in OPS is not a fork but a clone, allowing the OPS community to
easily update the OVS version to get bug fixes and new features. Contributing
to the OVS project is also seamless as individual patches can be sent for
upstreaming and then removed once they are part of the OVS code base.
