...
OpenVswitch (OVS) module is built in OpenSwitch (OPS) by downloading its
...
http://git.openswitch.net/cgit/openswitch/ops-build/tree/yocto/openswitch/meta-distro-openswitch/recipes-ops/openvswitch/ops-openvswitch).
...
by editing the file `ops-openvswitch.bb`.
...
checkout the branch where the new feature will be applied on.
```
git checkout patches/branch-<OVS_VERSION>
```
...
To combine two patches rebase functionality of git. The openswitch/ovs
