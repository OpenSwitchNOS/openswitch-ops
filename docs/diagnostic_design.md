...
The primary goal of the diagnostic dump module is to capture internal diagnostic information about features from related daemons.
...
- Basic diagnostics are dumped to a console or to a file  using the CLI.
...
### Sample code
#### BBscript
Add the dependency **DEPENDS = ops-supportability** in the BBScript of the respective daemon.
...
The daemon initialization function is required to register the handler for a basic diagnostics dump by calling **init_diag_dump_basic()**. The function **init_diag_dump_basic()** takes the diagnostic dump handler function name as an argument.
