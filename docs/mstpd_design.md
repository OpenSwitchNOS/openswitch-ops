...
When running in simulator mode, switchd communicates with Linux Bridge via stp container plugin. Only single instance MSTP is only supported in this mode.
...
###Columns used by ops-switchd to program Linux Bridge
    MSTP_Common_Instance_Port:
        port_state: used by ops-switchd to communicate to ASIC to Block/Forward.
