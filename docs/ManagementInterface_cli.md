...
This command is used to switch to management interface context.
#### Syntax ####
```
interface mgmt
```
#### Description ####
This command is used to switch to management interface. All the management interface commands are available in this context only.
#### Authority ####
Admin
#### Parameters ####
None
...
User can configure all valid IPv4 and IPv6 addresses. Reserved IP, Multicast IP, Broadcast IP and loopback addresses are not allowed. However, only one IP address can be configured per address family.
#### Examples ####
```
...
When “no” is specified, the default gateway is removed. If the user tries to remove a default gateway that was not configured, error message is displayed.
#### Examples ####
```
...
When "no" is specified the namserver targeted is removed. If user try to remove a nameserver that is not configured, error message is displayed. It is not possible to remove secondary nameserver without removing the primary nameserver.
#### Examples ####
```
