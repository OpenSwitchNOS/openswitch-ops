...
## ops-zebra
The ops-zebra is the RTM (Routing Table Manager) daemon that makes the decision regarding the best routes among the available routes learnt from other routing protocols and other types of routes like connected, static, and so on. The ops-zebra daemon learns the routes available from the different protocols and kind from the OVSDB "Route" table.
