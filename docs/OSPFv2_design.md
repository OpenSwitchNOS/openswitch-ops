...
The OSPF_Interface_Config table contains information related to interface related configurations, like hello interval, dead interval, authentication configuration.
...
Cmd-1 +---->|               +-------------------->|               |
            ...
            +---------------+                     +---------------+
            |   OSPF_       |                     |               |
         #0 | Interface_    |         #1          |  OSPF_        |
Cmd-1a +--->|  Config       +-------------------->|     Interface |
            |               |  OPS-OSPFD process  |               |
            +---------------+                     +---------------+
...
In above Figure-3, flow of the configuration through the OVSDB schema tables for the command to enable the ospf protocol on a subnetwork and interface level configuration is depicted. The numbers along the flow-arrows are indicative of the order of the flow.

The command (Cmd-1) will create a corresponding row in the "OSPF_Router" and an entry in the "networks" column. OPS-OSPFD process will then run OSPFv2 protocol on the network configured and will populate the corresponding rows in the "OSPF_Area" and "OSPF_Interface".
The command (Cmd-1a) will create a row in the "OSPF_Interface_Config". This configuration may impact the reltated columns of the corresponding "OSPF_Interface" table through OPS-OSPFD process.
...
In above Figure-4, flow of the configuration through the OVSDB schema tables for the command to configure a NBMA Neighbor is depicted. The numbers along the flow-arrows are indicative of the order of the flow.
