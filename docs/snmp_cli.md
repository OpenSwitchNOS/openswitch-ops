...
| **<1-65535>** | Required | Integer | The port on which the SNMP master agent listens for SNMP requests. Default is udp 161.|
...
| ** 1 or 2c ** | Required | 1 or 2c  | The SNMP trap notification version.|
...
```
[no] snmp-server host <A.B.C.D | X:X::X:X >  trap version < 3 > < auth | noauth | priv > user <WORD> [port <UDP port>]
```
...
| **A.B.C.D** | Required | A.B.C.D | Valid IPv4 address of the trap receiver.|
| **X:X::X:X** | Required | X:X::X:X | Valid IPv6 address of the trap receiver.|
| ** 3 ** | Required | 3 | The SNMP trap notification version.|
| **WORD** | Optional | String | The name of the community string to be used in the SNMP trap notifications. Default is public.|
| **UDP_port** | Optional | Integer | The port on which the trap receiver listens for SNMP trap notifications. Default is udp port 162.|
...
switch# show snmp
     ...
     Community Names :
      ...
      Trap Receivers:
      ---------------------------------------------------------------------
      Host            Port      Type             Community Name/SNMPv3 User
      ---------------------------------------------------------------------
     ...
     ---------------------------------
      ...
      SNMPv3 EngineID : 00:00:00:0b:00:00:a4:5d:36:48:cb:00 (system mac)
