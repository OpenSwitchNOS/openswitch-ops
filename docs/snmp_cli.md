...
```
[no] snmp-server host <A.B.C.D | X:X::X:X >  [trap | inform] [version < 1 | 2c >] [community WORD] [port <UDP port>]
```
...
| **1 or 2c** | Optional | 1 or 2c  | The SNMP protocol version. Default is SNMPv2c.|
...
(config)# snmp-server host 10.10.10.10 inform version 2c community public

(config)# no snmp-server host 10.10.10.10 inform version 2c community public

(config)# snmp-server host 10.10.10.10 inform version 2c community public port 5000

(config)# no snmp-server host 10.10.10.10 inform version 2c community public port 5000
...
```
[no] snmp-server host <A.B.C.D | X:X::X:X >  [trap | inform] [version < 3 >] < auth | noauth | priv > user <WORD> [port <UDP port>]
```
...
| **X:X::X:X** | Required | X:X::X:X | Valid IPv6 address of the trap receiver.|
| **trap or inform** | Optional | trap or inform  | The SNMP notification type. Default is trap.|
| **3** | Required | 3 | The SNMP trap notification version. To send SNMPv3 trap/inform need to configured with option 3.|
...
```
(config)# snmp-server host 10.10.10.10 trap version 3 auth user Admin

(config)# no snmp-server host 10.10.10.10 trap version 3 auth user Admin

(config)# snmp-server host 10.10.10.10 trap version 3 auth user Admin port 2000

(config)# no snmp-server host 10.10.10.10 trap version 3 auth user Admin port 2000
