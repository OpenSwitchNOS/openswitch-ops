...
- To enter IPv6 address family configuration mode for configuring routing protocols that use IPv6 address prefixes:
  *`router bgp < asn >`*, *`address-family < ipv6 >`*.
  ...
  The *`no address-family < ipv6 >`* is to exit from the ipv6 address family configuration mode.

- To advertise the IPv6 prefix network:
  *`router bgp < asn >`*, *`ipv6 bgp network < ipv6 >`*.
  Use this command in router configuration mode to advertise the specified prefix into the IPv6 BGP database.
  The *`no address-family < ipv6 >`* is to stop advertising the specified prefix into the IPv6 BGP database.

- To ensure comparison of MED for paths from neighbors in different autonomous systems:
  *`router bgp < asn >`*, *`bgp always-compare-med`*.
  Enabling the bgp 'always-compare-med' command ensures the comparison of the MED for paths from neighbors in different autonomous systems. The bgp 'always-compare-med command' is useful when multiple service providers or enterprises agree on a uniform policy for setting MED, though this is rare. When this is not the case, 'always-compare-med' may produce inconsistent results. The 'always-compare-med' setting is also useful when an AS uses MED for internal, intra-AS steering of equal-length AS_PATH routes to preferred egress points.
  The *`no bgp always-compare-med`* is to disable the comparison of the MED for paths from neighbors in different autonomous systems.

- To select the route with the lowest  router ID as the best path:
  *`router bgp < asn >`*, *`bgp bestpath compare-routerid`*.
  This command is used to configure a BGP routing process to compare identical routes received from different external peers during the best path selection process and to select the route with the lowest router ID as the best path. The default is to prefer existing routes over newer routes, to minimise flapping. Setting 'compare-routerid' will go on to check the router-ID instead.
  The *`no bgp bestpath compare-routerid`* is to return the BGP process to the default operation.

- To ensure comparison of MED for paths from neighbors in same autonomous systems:
  *`router bgp < asn >`*, *`bgp deterministic-med`*.
  Use this command in router configuration mode to ensures the comparison of the MED variable when choosing routes advertised by different peers in the same autonomous system.
  The *`no bgp deterministic-med`* is to disable the comparison of the MED for paths from neighbors in same. autonomous systems
...
- To enable advertising of the BGP network in IGP:
  *`router bgp < asn >`*, *`bgp network import-check`*.
  This command is used in router configuration mode to advertise BGP network in IGP.
  The *`no bgp network import-check`* is used to disallow  advertising of the BGP network in IGP.
...
- Enter the following neighbor configuration commands in the order shown:
        ...
        - Set the interface name when connecting to a BGP peer over an IPv6 link-local address.This command  may be deprecated in future releases.
          *`router bgp < asn >`*, *`neighbor peer interface <ifname>`*.
          The *`no neighbor peer interface <ifname>`* unsets the interface specified to connect to a BGP peer over an IPv6 link-local address.

        - Set strict comparison of remote capabilities and local capabilities with specified BGP peer. If capabilities are different, Unsupported Capability error is sent and connection is reset.
          *`router bgp < asn >`*, *`neighbor peer strict-capability-match`*.
          The *`no neighbor peer strict-capability-match`* unsets strict comparison of remote capabilities and local capabilities with specified BGP peer.

        - Activate specified neighboring peer to facilitate multicast routing updates.
          *`router bgp < asn >`*, *`neighbor peer activate`*.
          Prerequisite: If the peer is specified as a peer-group-name, peer-group-name needs to be configured first using *`neighbor <peer-group-name> peer-group`*.
          The *`no neighbor peer activate`* deactivates specified neighboring peer to facilitate multicast routing updates.

        - Facilitate support of negotiation of capabilities (sending new capabilities or removing previously negotiated capabilities) without performing a hard clear of the BGP session.
          The capability data field includes a list of capabilities that can be dynamically negotiated.
          *`router bgp < asn >`*, *`neighbor peer capability dynamic`*.
          The *`no neighbor peer capability dynamic`* disables negotiation of capabilities.

        - Facilitate support of cooperative route filtering to install a BGP speakers inbound route filter as an outbound route filter on the peer prefix-list. Installs the filter (any inbound prefix list or distribute list) as an outbound prefix list.
          *`router bgp < asn >`*, *`neighbor peer capability orf prefix-list <both|send|receive>`*.
          Prerequisite: Neighbor needs to be activated using *`neighbor  9.0.0.2 activate`* command.
          The *`no neighbor peer capability orf prefix-list <both|send|receive>`* uninstalls outbound route filter on BGP peer matching inbound route filter on BGP speaker set previously.

        - Suppress sending of capability negotiation as OPEN message optional parameter to the peer.
          This command only affects the peer is configured other than IPv4 unicast configuration. When remote peer does not have capability negotiation feature, remote peer will not send any capabilities at all. In that case, bgp configures the peer with configured capabilities. Locally configured capabilities are preferred to negotiated capabilities even though remote peer sends capabilities.
          *`router bgp < asn >`, `neighbor peer dont-capability-negotiate`*.
          The *`no neighbor peer dont-capability-negotiate`* enables negotiation of capabilities with peer.

        - Override the result of capability negotiation with local configuration to ignore remote peers capability value.
          *`router bgp < asn >`, `neighbor peer override-capability`*.
          The *`no neighbor peer override-capability`* enables configuration of capabilities negotiated with those of the peer.
        ...
        - Specify the communities attribute that should be sent to a BGP neighbor using the following commands.
          *`router bgp < asn >`, `neighbor (A.B.C.D|X:X::X:X|WORD) send-community (both|extended|standard)`*
          The *`no neighbor (A.B.C.D|X:X::X:X|WORD) send-community (both|extended|standard)`* removes the communities attributes.
...
- The command *`neighbor <ipv4_address | ipv6_address | peer_group_name> ebgp-multihop  <1-255>`* attempts to connect to the external Autonomous System routers which are not directly connected. It take either IPv4/IPv6 address or the peer-group name to establish a connection. *`ebgp-multihop <1-255>`* option is used to set the hop count value. Parameter *`ebgp-multihop`* sets the time-to-live hop count value for the BGP speaking router.
