...
- Following are the route map configuration commands:
 ...
 - Exclude the given AS number from the AS_PATH with the following command:
   ...
   The <value> variable specifies the AS to be excluded.  The range is from 1 to 4294967295.
 ...
 - Prepend the given AS number to the AS_PATH with the following command:
   ...
   The <value> variable specifies the AS to be prepended to the AS path. The range is from 1 to 4294967295.
 ...
 - The following command removes the COMMUNITY attributes from the BGP routes identified in the specified community list. It also deletes matching communities from the route map.
   ...
   ```
   Communities can be set for the route map using the following:
   ```
   set community .AA:NN
   ...
   ```
The <value> variable can be any of the following:

- Standard community with a range of 1 to 99
- Extended community in the range of 100 to 500
- A community name
 ...
 - Set the COMMUNITY attributes for a route map with the following commands:
   `route-map <word> (deny|permit) <sequence_num>`, `set community <community-number>`

The community number parameter can be in one of the following formats:
- aa:nn
...
The `no set community <community-number>` command removes the community attribute for the route map.
 ...
 - Use the following commands to set the BGP route site-of-origin extended community (in decimal notation).
 ...
 - In the BGP routing protocol, the update from the external AS contains the next hop address of the external hop itself. Therefore, when the router receives the update from the external AS, and advertises the update to the other routers via IBGP, the other routers see the next-hop of the advertised networks via the external router IP address.
  Enable an IBGP learn router to forward a packet to its peer address instead of the external router IP address using the following commands:
   ...
   The `no set ip next-hop peer-address` command prevents forwarding BGP route update packets to the peer as a next-hop.

 - Set the BGP-4+ global IPv6 next hop address using the following commands:
 ...
 - Set the BGP-4+ link-local IPv6 next hop address using the following commands:
 ...
 - Set the local preference value of an IBGP route, which is advertised to IBGP peers, using the following commands:
 ...
 - Specify the relative change of metric, which is used with BGP route advertisement, using the following commands:
   ...
   The expr parameter is the metric expression to increment or decrement the metric of a route using the "+/-*metric-value*" syntax.
 ...
 - Set the metric type for the destination routing protocol using the following commands:
   ...
   The type attribute is either type-1 or type-2.
 ...
 - Set the ORIGIN attribute of a local BGP route using the following commands:
   ...
   The origin-type attribute is one of the following:
 ...
 - Set the ORIGINATOR_ID attribute, which is equivalent to the router-id of the route originator in the local AS, using the following commands:
 ...
 - Use the following commands to set the tag for redistributions that external routers use to filter incoming distributions of route-maps:
   ...
   The value attribute is in the range 0 - 65535.
 ...
 - Set the weight of a BGP route, using the following commands:
  ...
  The weight of the route has the most influence when two identical BGP routes are compared. A higher number signifies a greater preference.
   The value attribute is in the range 0 - 4294967295.
 ...
 - To match a BGP extended community list, use the following commands:
 ...
 - To match an IP address, use the following commands:
   ...
   The `no match ip address (<1-199>|<1300-2699>|WORD)` command removes the match ip address entry.
...
- Enter the following peer filtering ip prefix-list commands in the order shown:
