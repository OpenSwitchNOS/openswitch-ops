...
For changing the priority of a session with the IDL run the command
...
the OVSDB Server can be retrieved (after a ovsdb_idl_run).
...
That file can be updated and reloaded in runtime using ovs-appctl:
```
