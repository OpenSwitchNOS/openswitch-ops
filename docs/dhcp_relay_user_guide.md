...
three components: the DHCP client, the DHCP server, and the DHCP relay agent. The DHCP client sends broadcast request packets to the network and DHCP servers respond with broadcast
packets that offer IP parameters, such as an IP address for the client. After the client chooses the IP parameters, communication between the client and server is by unicast packets.
...
the routing switch forwards DHCP client packets to all DHCP servers (Helper IP addresses) that are configured in the table administrated for each interface. The helper address
configuration is allowed only on data plane interfaces.

â€¢ DHCP relay option 82
Option 82 is called the relay agent information  option. Option 82 field is inserted/replaced or the packet with this option is dropped by the DHCP relay agent, when forwarding client-originated DHCP packets to a DHCP server. Servers recognizing the relay
agent information option may use the information to implement IP address or other parameter assignment policies. The relay agent relays the server-to-client replies to the client.

â€¢ Hop count in DHCP requests
...
â€¢ Configuring a BOOTP/DHCP relay gateway
...
â€¢   IPv4-address - The IPv4 address of the protocol server. This is a unicast address of a destination server on another subnet. The maximum number of helper-addresses which can be configured per
                   interface is 8. DHCP relay functions on L3 interfaces that includes split-interface and sub-interfaces.

`show ip helper-address [interface <interface-name>]`
* Displays the configured IP helper-address(es).*

Explanation of parameters
â€¢   interface `<interface-name>` - Interface on which server addresses are configured.
...
Disable response validation alone for drop/replace policy.  Not applicable if keep policy was selected.

`no dhcp-relay option 82 [validate]`

Explanation of parameters

â€¢ drop      - Configures the router to unconditionally drop any client DHCP packet received with existing
              option 82 fields. This means that such packets will not be forwarded. Use this option where access to
              the router by untrusted clients is possible.
â€¢ keep      - For any client DHCP packet received with existing option 82 fields, configures the router to forward the packet as-is, without replacing or adding to the existing
              option 82 fields.
â€¢ replace   - Configures the switch to replace existing option 82 fields in an inbound client DHCP packet with an Option 82 field for the switch.
â€¢ validate  - Operates when the routing switch is configured with append, replace, or drop as a forwarding policy. With validate
              enabled, the routing switch applies stricter rules to an incoming Option 82 server response to determine whether to forward or drop the response.
â€¢ ip        - Specifies the IP address of the interface on which the client DHCP packet enters the switch.
â€¢ mac       - Specifies the router's MAC address. (The MAC address used is the same MAC address that is assigned to all interfaces configured on the router.) This is the default setting.


`show dhcp-relay`
* Displays the dhcp-relay option 82 configurations.*
...
Explanation of parameters
â€¢   IPv4-address - The IPv4 address of the gateway. Provides a way to configure a gateway address for the DHCP relay agent
...
* Displays the configured bootp-gateway.*

Explanation of parameters
â€¢   interface `<interface-name>` - Interface on which gateway address is configured.
...
* Enable/Disable dhcp-relay hop-count-increment. By default, it is enabled.*
