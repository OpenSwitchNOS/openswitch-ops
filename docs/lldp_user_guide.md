...
3. Setting the LLDP Reinit Delay.
...
ops-as5712(config)#
```
The `no lldp reinit` commands reverts the LLDP reinit delay to its default value of two seconds.
```
ops-as5712(config)# no lldp reinit
ops-as5712(config)#
```

4. Setting the ip to be used in the Management Address TLV.
The `lldp management-address` command specifies the ip to be used in  management address LLDP type-length-value (TLV) triplets. If this command is not configured, ip address assigned to management interface will be used in  management address LLDP type-length-value (TLV) triplets.
```
...
The 'show lldp local-device' command displays the information advertised by the switch if LLDP feature is enabled by user.
```
