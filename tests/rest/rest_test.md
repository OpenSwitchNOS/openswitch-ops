...
> 4. Configure the system through Standard REST API PUT Method for the URI "/rest/v1/system".
> 5. Validate the system configuration with HTTP return code for the URI "/rest/v1/system".
> 6. Execute Standard REST API GET Method for URI "/rest/v1/system".
> 7. Validate the GET Method HTTP return code for "/rest/v1/system" and respective values.
...
> 3. Configure the IPV4 address on the Ubuntu workstation.
> 4. Execute Standard REST API GET Method for URI "/rest/v1/system/subsystems".
> 5. Validate the GET Method HTTP return code for "/rest/v1/system/subsystems" and respective values.

### Test result criteria ###
#### Test pass criteria ####
The test case is passes if the standard REST API GET method returns HTTP code 200 for the URI "/r est/v1/system/subsystems" and the returned data is identical.
#### Test fail criteria ####
...
> 2. Validate the "/rest/v1/system/interfaces/{id}" configuration with HTTP return code.
> **Test 2**
> 1. Execute Standard REST API GET Method for URI "/rest/v1/system/interfaces/{id}".
> 2. Validate the GET Method HTTP return code for "/rest/v1/system/interfaces/{id}" and respective values.
> **Test 3**
...
### Objective ###
The objective of this test case is to validate the "/rest/v1/system/vrfs" through the standard REST API G ET method.
### Requirements ###
The requirements for this test case are:
- OpenSwitch.
- Ubuntu Workstation.
...
> 2. Configure the IPV4 address on the switch management interfaces.
> 3. Configure the IPV4 address on the Ubuntu workstation.
> 4. Execute Standard REST API GET Method for URI "/rest/v1/system/vrfs".
> 5. Validate the GET Method HTTP return code for "/rest/v1/system/vrfs" and respective values.

### Test result criteria ###
#### Test pass criteria ####
The test case is passes if the standard REST API GET method returns HTTP code 200 for the URI "/r est/v1/system/vrfs" and if the returned data is identical.
#### Test fail criteria ####
...
```
### Description ###
The objective of the test case is to validate the "/rest/v1/system/route_maps/{id}" through the standard REST API GET method.
> **STEPS:**

> 1. Connect the OpenSwitch to Ubuntu workstation as shown in the topology diagram.
> 2. Configure the IPV4 address on the switch management interfaces.
> 3. Configure the IPV4 address on the Ubuntu workstation.
> 4. Execute Standard REST API GET Method for URI "/rest/v1/system/route_maps/{id}".
> 5. Validate the GET Method HTTP return code for "/rest/v1/system/route_maps/{id}" and respective values.
...
> 2. Configure the IPV4 address on the switch management interfaces.
> 3. Configure the IPV4 address on the Ubuntu workstation.
> 4. Execute Standard REST API GET Method for URI "/rest/v1/system/interfaces".
> 5. Validate the GET Method HTTP return code for "/rest/v1/system/interfaces" and respective values.
...
```
### Description ###
Configure the REST API PUT method with invalid data for URIs..
...
> 5. Validate that standard REST API PUT Method fails to configure with invalid data for all URIs.
...
#### Test fail criteria ####
This test case is fails if the standard REST API PUT method with invalid data returns HTTP code 200 OK for the URIs.
...
> 3. Configure the IPV4 address on the Ubuntu workstation.
> **Test 1**
> 1. Execute Standard REST API POST Method for URI "/login" with valid credentials.
> 2. Validate the GET Method HTTP return code for URI "/login".
> **Test 2**
> 1. Execute Standard REST API POST Method for URI "/login" with invalid credentials.
> 2. Validate the GET Method HTTP failed return code for URI "/login".
...
- b. Verify if the response data is not empty
- c. Verify if the response data has the keys: "configuration", "status" and "statistics"
- d. Verify if the configuration data is equal to the Port1 configuration data

3. Verify if a non-existent port exists
- a. Execute GET request over rest/v1/system/ports/Port2
...
-Querying a port list for:
      ...
      -A URI "rest/v1/system/ports/Port1" in the port list returned from rest/v1/system/ports URI.
...
- Querying Port1 for:
   ...
   - A response data that contains keys: "configuration", "status",
     and "statistics".
   - Preset port configuration data is equal to the Port1.
...
- Querying a post list for:
   - An HTTP response not equal to 200 OK
...
- Port data validation such as ranges, types, allowed values, malformed JSON, and missing attributes.
...
Verify that the HTTP response returns BAD_REQUEST when creating a existing port with name "Port1".
...
-The test is considered passed for "creating a new port with an invalid value on an attribute" when the HTTP response is 400 BAD REQUEST.
...
- The configuration data posted is not the same that the retrieved port.
...
The test is failing for "creating a new port with an invalid array type" when the HTTP response is not equal 400 BAD REQUEST.
...
The test is failing for "creating a new port with with all the attributes" when the HTTP response is not equal 201 CREATED.

The test is failing for "creating a new port with an unknown attribute" when the HTTP response is not equal 400 BAD REQUEST.
...
The test is failing for "creating a new port with malformed json data" when the HTTP response is not equal 400 BAD REQUEST.

The test is failing for "creating a new port with well-formed json data" when the HTTP response is not equal 201 CREATED.
...
3. confirm that the configuration response data from the Step 2 is the same as the configuration data from Step 1.
...
1. Execute a PUT request with /rest/v1/system/ports/Port1 and without "vlan_mode" attribute.
2. Verify that the HTTP Response is 400 BAD REQUEST.
...
The test is passing for "updating a port with valid array type" when the HTTP response is 200 OK.

The test is passing for "updating a port with an invalid value on an attribute" when the HTTP response is 400 BAD REQUEST.

The test is passing for "updating a port with a valid value on an attribute" the HTTP response is 200 OK.
...
The test is passing for "updating a port with all the allowed attributes" the HTTP response is 200 OK.

The test is passing for "updating a port with malformed json data" the HTTP response is 400 BAD REQUEST.
...
The test is failing for "updating a port with valid array type" when the HTTP response is not equal to 200 OK.
...
- There is no URI "/rest/v1/system/ports/Port1" in the port list that is returned from /rest/v1/system/ports URI.
...
- When performing a GET request on "/rest/v1/system/ports", "Port1" is displayed in the ports URI list. -The HTTP response is not equal to 404 NOT FOUND when doing a GET request on "/rest/v1/system/ports/Port1".
