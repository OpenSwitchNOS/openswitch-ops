...
class QuerySortPortTest (OpsVsiTest):
    ...
    **********************************************************************
    ...
    def execute_sort_by_request(self, attributes, expected_code, desc=False,
        ...
        if filtered_ports is not None:
            if isinstance(filtered_ports, dict):
                for key in filtered_ports.viewkeys():
                    path += key + "=" + filtered_ports[key]
                path += ";"
            else:
                path += filtered_ports[key] + ";"
