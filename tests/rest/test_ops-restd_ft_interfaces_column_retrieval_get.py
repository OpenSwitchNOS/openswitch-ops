...
class ColretrieveInterfaceTest(OpsVsiTest):
    ...
    def get_columns_to_retrieve(self, columns):
        ...
        if columns:
            for column in columns:
                appended_columns = column + "," + appended_columns
            appended_columns = list(appended_columns)
            del(appended_columns[len(appended_columns)-1])
            appended_columns = ''.join(appended_columns)
        return appended_columns

    def validate_retrieved_data_by_columns(self, complete_data_rows, data_rows,
                                           columns):
        if not (len(complete_data_rows) > 0 or len(data_rows) > 0 or
                len(columns) > 0):
            return False
        for i in [i for i, row in enumerate(data_rows)]:
    ...
    def test_single_column_retrieval(self):
        ...
        complete_data_rows = self.check_malformed_json(response_data)

        response, response_data = execute_request(new_path, "GET", None,
                                                  self.switch_ip, True)
        status_code = response.status
        assert status_code == httplib.OK, "Wrong status code %s " % status_code
        data_rows = self.check_malformed_json(response_data)
        info("### Data successfully retrieved. Status code 200 OK ###\n")
        # Test
        # 2 - Validate that data exist in rows by columns retrieval
        self.validate_retrieved_data_by_columns(complete_data_rows, data_rows,
                                                columns)
    ...
    def test_column_retrieval_with_invalid_column_key(self):
        # Test Setup
...
class Test_ColretrieveInterface:
    ...
    def test_run_call_test_column_retrieval_with_invalid_column_key(self):
        self.test_var.test_column_retrieval_with_invalid_column_key()
