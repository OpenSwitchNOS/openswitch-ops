...
def restTestInterfaces(wrkston01):
    ...
    restruct_intf1 = retStruct.data['response_body'].split(",")[0].split("[")[1].strip('\'')
    assert retStruct.data[
        "response_body"].find(restruct_intf1) != -1, 'Failed\
          in checking the GET METHOD JSON response validation for Interface '\
          + restruct_intf1
    info('### Success in Rest GET for Interface ' + restruct_intf1.split("/")[5] + '###\n')
    retStruct = returnStruct(returnCode=0)
    return retStruct
