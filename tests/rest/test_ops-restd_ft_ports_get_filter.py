...
class QueryPortTest (OpsVsiTest):
    ...
    def test_port_filter_by_name(self):
        ...
        info("\n########## Test to filter port by name  ##########\n")

        for i in range(1, NUM_FAKE_PORTS +1):
            test_port = "Port-%s" % i
            path = "%s?depth=1;%s=%s" % (self.path, test_field, test_port)

            request_response = self.validate_request(self.switch_ip, path, None, "GET", httplib.OK, "")
        ...
        info("########## End Test to filter port by name ##########\n")
    ...
    def test_port_filter_by_interfaces(self):
        ...
        info("\n########## Test to filter port by interface  ##########\n")

        #######################################################################
        # Update port values
        ######################################################################
        for port in test_ports:
            update_test_port(self.switch_ip, self.path + port, test_field, test_new_value)
        ...
        assert len(request_response) == updated_ports, "Retrieved more ports than expected!"
        ...
        assert len(request_response) == other_ports, "Retrieved more ports than expected!"
        ...
        #######################################################################
        for port in test_ports:
            update_test_port(self.switch_ip, self.path + port, test_field, test_old_value)

        info("########## End Test to filter port by interface ##########\n")
    ...
    def test_port_filter_by_trunks(self):
        ...
        info("########## End Test to filter port by trunk ##########\n")

    def test_port_filter_by_primary_ip4_address(self):
        test_field = "ip4_address"

        info("\n########## Test to filter port by Primary IPv4 Address  ##########\n")
        ...
        for i in range(1, NUM_FAKE_PORTS +1):
            ...
            assert len(request_response) is 1, "Retrieved more expected ports!"
            assert request_response[0]["configuration"][test_field] is not test_ipv4, "Retrieved wrong port!"

        info("########## End Test to filter port by Primary IPv4 Address ##########\n")

    def test_port_filter_by_secondary_ipv4_address(self):
        test_field = "ip4_address_secondary"

        info("\n########## Test to filter port by Secondary IP4 Address  ##########\n")
        ...
        info("########## End Test to filter port by Secondary IP4 Address ##########\n")
    ...
    def test_port_filter_by_lacp(self):
        ...
        info("\n########## Test to filter port by lacp ##########\n")

        #######################################################################
        # Update values
        #######################################################################
        update_test_port(self.switch_ip, self.path + "/Port-1", test_field, ["passive"])
        update_test_port(self.switch_ip, self.path + "/Port-2", test_field, ["off"])
        ...
        for i in range(0, other_ports):
            assert request_response[i]["configuration"][test_field] == "active", "Retrieved wrong port!"

        #######################################################################
        # Restore default value
        #######################################################################
        update_test_port(self.switch_ip, self.path + "/Port-2", test_field, ["active"])
        update_test_port(self.switch_ip, self.path + "/Port-3", test_field, ["active"])

        info("########## End Test to filter port by lacp ##########\n")
    ...
    def test_port_filter_by_bond_mode(self):
        ...
        info("\n########## Test to filter port by bond_mode ##########\n")
        ...
        info("########## End Test to filter port by bond_mode ##########\n")
    ...
    def test_port_filter_by_bond_active_slave(self):
        ...
        info("\n########## Test to filter port by Bond Active Slave ##########\n")
        ...
        info("########## End Test to filter port by Bond Active Slave ##########\n")
    ...
    def test_port_filter_by_tag(self):
        ...
        info("########## End Test to filter port by tag ##########\n")
    ...
    def test_port_filter_by_vlan_mode(self):
        ...
        info("\n########## Test to filter port by vlan_mode ##########\n")

        #######################################################################
        # Update values
        #######################################################################
        for i in range(0, updated_ports):
            update_test_port(self.switch_ip, self.path + test_ports[i], test_field, test_new_value[i])
        ...
        for mode in test_new_value:
            ...
            assert len(request_response) == 1, "Retrieved more ports than expected!"
        ...
        info("########## End Test to filter port by vlan_mode ##########\n")

    def test_port_filter_by_mac(self):
        test_field = "mac"

        info("\n########## Test to filter port by MAC ##########\n")
        ...
        for i in range(1, NUM_FAKE_PORTS +1):
            ...
            assert len(request_response) is 1, "Retrieved more expected ports!"
            assert request_response[0]["configuration"][test_field] is not test_mac, "Retrieved wrong port!"

        info("########## End Test to filter port by MAC ##########\n")

    def test_port_filter_by_ipv6_address(self):
        test_field = "ip6_address"

        info("\n########## Test to filter port by Primary IPv6 Address ##########\n")
        ...
        for i in range(1, NUM_FAKE_PORTS +1):
            ...
            assert len(request_response) is 1, "Retrieved more expected ports!"
            assert request_response[0]["configuration"][test_field] is not test_ip6, "Retrieved wrong port!"

        info("########## End Test to filter port by Primary IPv6 Address ##########\n")

    def test_port_filter_by_ipv6_address_secondary(self):
        test_field = "ip6_address_secondary"

        info("\n########## Test to filter port by Secondary IPv6 Address ##########\n")
        ...
        for i in range(1, NUM_FAKE_PORTS +1):
            ...
            assert len(request_response) is 1, "Retrieved more expected ports!"
            assert request_response[0]["configuration"][test_field] is not secondary_ip6, "Retrieved wrong port!"

        info("########## End Test to filter port by Secondary IPv6 Address ##########\n")
    ...
    def test_port_filter_by_admin(self):
        ...
        info("########## End Test to filter port by Admin ##########\n")

    def setup_switch_ports(self, total):
        for i in range(1, total+1):
            create_fake_port(self.path, self.switch_ip, i)

    def validate_request(self, switch_ip, path, data, op, expected_code, expected_data):
        status_code, response_data = execute_request(path, op, data, switch_ip)

        assert status_code is expected_code, "Wrong status code %s " % status_code
        #info("### Status code is OK ###\n")

        assert response_data is not expected_data, "Response data received: %s\n" % response_data
    ...
    def run_tests(self):
        ...
        """
        methodlist = [n for n, v in inspect.getmembers(self, inspect.ismethod) if isinstance(v,types.MethodType)]
...
class Test_QueryPort:
    ...
    def setup_class (cls):
        ...
        Test_QueryPort.test_var.setup_switch_ports(NUM_FAKE_PORTS)
