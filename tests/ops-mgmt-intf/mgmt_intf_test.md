...
- [IPv4 DCHP mode](#verifying-management-interface-configuration-test-cases-in-ipv4-dhcp-mode.)
...
#### Pass Criteria ####
The test case is successful if the dhcpclient service state is in a running state.
#### Fail Criteria ####
The test case is a fails if the dhcpclient is not in a running state.

### Test case 1.02 - Verifying that the management interface is updated from the `image.manifest`.###
#### Description ####
...
#### Fail Criteria ####
The test case  is a fails if the `name=eth0` is missing from the mgmt_intf column.
...
### Test Result Criteria ###
#### Pass Criteria ####
The test is successful if the following cirterions are met:
...
- The dhcpc client service is running.
...
```ditaa
              ...
              +------------------+     |         |         +-------------------+
                                 ...
                                 +---------------------+
```
### Test case 2.01 - Verifying that the static IPV4 addresses are configured on the management interface. ##
#### Description ####
...
#### Description ####
Configure the static default IPv4 gateway in the management interface using the management interface con text.
### Test Result Criteria ###
#### Pass Criteria ####
...
### Test Result Criteria ###
#### Pass Criteria ####
The test is successful if the `Default gateway IPv4` address is missing in the `show int erface mgmt` or the `ip route show` ouput
#### Fail Criteria ####
