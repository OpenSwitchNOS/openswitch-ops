...
#
###############################################################################
# Name:        lagStaticModifyNumberMembers
#
# Description: To verify it is possible to modify static LAG's lower limits
#
# Author:      Pablo Araya M.
#                Adapted from DynamicLagModifyLowNumberOfMembers by Jose Hernandez
#
# Topology:  |Host| ----- |Switch| ---------------------- |Switch| ----- |Host|
#                                   (Dynamic LAG - 3 links)
#
# Success Criteria:  PASS -> LAGs are modified to support 0, 1, 2 or 3 members
#                            and pass traffic
#
#                    FAILED -> LAGs cannot be modified to 0, 1, 2 or 3 members or
...
class Test_lagStaticModifyNumberMembers:

    def setup_class(cls):
        # Create Topology object and connect to devices
        Test_lagStaticModifyNumberMembers.testObj = testEnviron(topoDict=topoDict)
        Test_lagStaticModifyNumberMembers.topoObj =\
            Test_lagStaticModifyNumberMembers.testObj.topoObjGet()
    ...
    def teardown_class(cls):
        ...
        # Terminate all nodes
        Test_lagStaticModifyNumberMembers.topoObj.terminate_nodes()

    def test_createLAGs(self):
        LogOutput('info', "\n############################################")
        LogOutput('info', "Create LAG 1 on both switches and assign 3 interfaces to it")
        LogOutput('info', "############################################")
    ...
    def test_modifyLAGs2(self):
        LogOutput('info', "\n############################################")
        LogOutput('info', "Remove second LAG member")
        LogOutput('info', "############################################")
        dut01Obj = self.topoObj.deviceObjGet(device="dut01")
        dut02Obj = self.topoObj.deviceObjGet(device="dut02")
        LogOutput('info', "Delete 1 LAG member from dut01")
    ...
    def test_modifyLAGs3(self):
        LogOutput('info', "\n############################################")
        LogOutput('info', "Remove third LAG member")
        LogOutput('info', "############################################")
        dut01Obj = self.topoObj.deviceObjGet(device="dut01")
        dut02Obj = self.topoObj.deviceObjGet(device="dut02")
