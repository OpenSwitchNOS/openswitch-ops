...
def enableLagShutdown(deviceObj, lag, enable):
    overallBuffer = []
    ...
    if returnCode != 0:
        ...
        return returnCls
    # enter interface
    command = "interface %s\r" % lag
    returnDevInt = deviceObj.DeviceInteract(command=command)
    retCode = returnDevInt['returnCode']
    overallBuffer.append(returnDevInt['buffer'])
    if retCode != 0:
        LogOutput('error', "Failed to configure " +
                  lag + " on device " + deviceObj.device)
        bufferString = ""
        for curLine in overallBuffer:
            bufferString += str(curLine)
    ...
    if enable:
        ...
        overallBuffer.append(returnDevInt['buffer'])
        if retCode != 0:
            LogOutput('error', "Failed to no shutdown " +
                      lag + " on device " + deviceObj.device)
            bufferString = ""
            for curLine in overallBuffer:
                bufferString += str(curLine)
            ...
            LogOutput('info', "Enabled no shutdown on " +
                      lag + " on device " + deviceObj.device)
    else:
        # configure interface
        command = "shutdown"
        ...
        overallBuffer.append(returnDevInt['buffer'])
        if retCode != 0:
            LogOutput('error', "Failed to shutdown " +
                      lag + " on device " + deviceObj.device)
            bufferString = ""
            for curLine in overallBuffer:
                bufferString += str(curLine)
...
def configureWorkstations(self):
    LogOutput('info', "\n############################################")
...
class Test_lagDynamicNonConsecutiveInterfaces:
    ...
    def lagShutdown(self):
        LogOutput('info', "\n############################################")
        LogOutput('info', "Test lag shutdown")
        LogOutput('info', "############################################")
        wrkston01Obj = self.topoObj.deviceObjGet(device="wrkston01")
        wrkston02Obj = self.topoObj.deviceObjGet(device="wrkston02")
        dut01Obj = self.topoObj.deviceObjGet(device="dut01")
        ...
        createLAGs(self)
        configureVLANs(self)
        enableDUTsInterfaces(self)
        configureWorkstations(self)
        pingBetweenClients(self)

        retStruct = enableLagShutdown(dut02Obj, "lag 1", False)
        if retStruct.returnCode() != 0:
            return False
    ...
    def test_lagNoShutdown(self):
        LogOutput('info', "\n############################################")
        LogOutput('info', "Test lag no shutdown")
        LogOutput('info', "############################################")
        wrkston01Obj = self.topoObj.deviceObjGet(device="wrkston01")
        wrkston02Obj = self.topoObj.deviceObjGet(device="wrkston02")
        dut01Obj = self.topoObj.deviceObjGet(device="dut01")
        ...
        retStruct = enableLagShutdown(dut02Obj, "lag 1", True)
        if retStruct.returnCode() != 0:
            return False
