...
#
###############################################################################
# Name:        test_lacp_ft_set_system_id.py
#
# Description: Tests that the System-Priority for LACP links can be changed to
#              valid values (range is from 0 through 65535) and verify the
#              received System-Priority from the neighbor switch
#
# Author:      Jose Calvo
#
...
def set_priority(dev, value, set_unset):
...
# the switch configuration
    def test_set_system_priority(self):
        ...
        for i in range(0, 2):
            flag = True
            ret_code = set_priority(dut01Obj, str(values[i]), flag)
            ...
            val_config = validate_system(lacp_output, str(values[i]))
            ...
            LogOutput('info', "Configuring default System-Priority...\n")
            ...
            lacp_output = lagpGlobalSystemShow(deviceObj=dut01Obj).buffer()
            val_config = validate_system(lacp_output, '65534')

            if not val_config:
                LogOutput('error', "Could not set Default System-Priority\n")
                test = False
                break
            ...
            if not lacp_setup:
                ...
                test = False
                break
            LogOutput('info', "Default LACP System-Priority set successfully")
            LogOutput('info', "\n")
