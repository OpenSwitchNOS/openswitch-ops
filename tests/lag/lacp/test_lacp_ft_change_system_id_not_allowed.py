...
#
###############################################################################
# Name:        test_lacp_ft_change_system_id_not_allowed.py
#
# Description: Tests that the System-Priority for LACP links cannot be changed
#              to invalid values (range is from 0 through 65535)
#
# Author:      Jose Calvo
#
...
#                                  (Dynamic LAG - 2 links)
#
# Success Criteria: PASS -> System-Priority cannot be changed to invalid values
#
#
#
#                   FAILED -> LACP System-Priority is set with an invalid value
#
#
###############################################################################
...
# Gets the random value and apply it to the LACP system-priority
...
# Generates random values to be applied to the switch configuration
# Random values include: negative values, floating point numbers
# And positive integers greater than 65535. Values are added to a list
def gen_ran_values():
    list_val = []
    for i in range(0, 10):
        ran_value = random.randint(1, 3)
        if ran_value == 1:
            value = random.randint(65536, 200000)
        elif ran_value == 2:
            value = random.uniform(0, 65535)
        else:
            value = random.randint(0, 65535) * -1
...
# Random invalid values are generated and applied to
# the switch configuration
    def test_set_system_priority(self):
        ...
        for i in range(0, 10):
            ...
            if val_config:
                LogOutput('error', "Incorrect LACP System-Priority set\n")
                test = False
                break
            ...
            if not lacp_setup:
                ...
                test = False
                break
            LogOutput('info', "LACP System-Priority not set - invalid value")
            LogOutput('info', "\n")

        if not test:
            LogOutput('error', "       STEP 6 COMPLETE - FAILED      ")
        else:
            LogOutput('info', "       STEP 6 COMPLETE - SUCCESS      ")
        LogOutput('info', "\n")
