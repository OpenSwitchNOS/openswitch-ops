...
def generateIPAddress(ipAddrList, prefix, mask=24, maxList=254):
    ...
    for i in prefixList:
        ...
        prefixBin += number
    complementMask = pow(2, 32 - mask)
    random.seed()
    while True:
        if len(ipAddrList) >= maxList:
            return None
        val = random.randint(1, complementMask - 2)
        ipBin = val + prefixBin
        ipAddrList = []
        for i in range(0, 4):
            octet = ipBin & 255
            ipAddrList.insert(0, octet)
            ipBin = ipBin >> 8
        ipAddr = '{}.{}.{}.{}'.format(ipAddrList[0], ipAddrList[1],
                                      ipAddrList[2], ipAddrList[3])
