...
#
###############################################################################
# Name:        test_lag_static_ft_tagged_vlans.py
#
# Description: Tests that different tagged traffic are allowed in a static
#              trunk
#
...
#                                   (Static LAG - 2 links)
#
# Success Criteria:  PASS -> Traffic flows between tag ports
#
#                    FAILED -> Traffic don't flow between tag ports
#
###############################################################################
import pytest
import re
...
class Test_ft_LAG_Static_tagged_vlans:
    ...
    def test_configure_interface_lag(self):
        ...
        for interfaceDut in listInterfacesDut2:
            ...
            if devIntLagRetStruct.returnCode() != 0:
                ...
                LogOutput('info', "Passed interface lag id configured ")
        # FIXME add LoadBalancing validation for Static trunks
        """
        assert(
            lagVerifyTrafficFlow(
                dut01Obj,
                dut02Obj,
                wrkston01Obj,
                wrkston02Obj,
                '140.1.1.10', #Change IP address
                '140.1.1.11', change IP address
                ['lnk02', 'lnk03'],
                'lnk01',
                'lnk04',
                '140.1.1'
    ...
    ##########################################################################
    # Step 5 - Configured vlan
    ##########################################################################

    def test_configure_vlan(self):

        LogOutput('info', "\n###############################################")
        LogOutput('info', "# Step 5 - Configure vlan  in the switch")
        LogOutput('info', "###############################################")
    ...
    def test_interface_vlan(self):
        ...
        LogOutput('info', "\n###############################################")
        LogOutput('info', "# Step 6 - Configure vlan in  the interface")
        LogOutput('info', "###############################################")
