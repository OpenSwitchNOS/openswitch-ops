...
def createLAGNegative(deviceObj, lagId, configure=True):
    '''
    Configures a LAG with a invalid ID to verify if DUT rejects the parameter
...
class Test_ft_framework_basics:
    ...
    def test_verifyInterfaces(self):
        ...
        for dut in [dut01, dut02]:
            for link in dut['links']:
                assert(interfaceGetStatus(
                    dut['obj'], dut['obj'].linkPortMapping[link]))
...
# Negative Testing
    ...
    def test_createLAGs_Negative(self):
        # Configure invalid LAGs in the DUT
        LogOutput('info', "\n############################################")
        LogOutput('info', "Configure a new LAG with invalid IDs")
        LogOutput('info', "############################################")
        dut01Obj = self.topoObj.deviceObjGet(device="dut01")
        dut02Obj = self.topoObj.deviceObjGet(device="dut02")
    ...
    def test_verify_lag_number(self):
        ...
        LogOutput('info', "\n############################################")
        LogOutput('info', "Verify number of LAGs configured")
        LogOutput('info', "############################################")
        dut01Obj = self.topoObj.deviceObjGet(device="dut01")
        dut02Obj = self.topoObj.deviceObjGet(device="dut02")
        assert(lagVerifyNumber(dut01Obj, 1))
