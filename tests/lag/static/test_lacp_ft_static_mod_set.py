...
#
###############################################################################
# Name:        test_lacp_ft_static_mode_set.py
#
...
def dev_cleanup(dev1, dev2):
    results = []
    dut01 = {'obj': dev1, 'links': ['lnk01', 'lnk03', 'lnk04', 'lnk05'],
             'wrkston_links': ['lnk01']}
    dut02 = {'obj': dev2, 'links': ['lnk02', 'lnk03', 'lnk04', 'lnk05'],
             'wrkston_links': ['lnk02']}
...
# Handles a thread in charge of sending packets while the switch configuration
# takes place
class TrafficCheck (threading.Thread):
    ...
    def run(self):
        ...
        while self.flag is True:
            ...
            ret_code = ping_host(self.host, "192.168.1.11")
            try:
                validate_step(ret_code, 7)
...
# Step 4: Create dynamic LAG and assign it to switch interfaces
    def test_create_lag(self):
        ...
        ret_code = lag_create_static(devices)
...
# Step 7: Apply LAG config while packets traverse the links
    def test_reapply_lag_send(self):
        LogOutput('info', "\n")
        LogOutput('info', "###################################")
        ...
        if not test:
            ...
            LogOutput('info', "\n")
            LogOutput('info', "Traffic sent while modifying the configuration")
            LogOutput('info', "\n")
...
# Step 8: Device cleanup
    def test_device_cleanup(self):
        LogOutput('info', "\n")
        LogOutput('info', "#######################")
        LogOutput('info', "STEP 8 - DEVICE CLEANUP\n")
        dut01Obj = self.topoObj.deviceObjGet(device="dut01")
        dut02Obj = self.topoObj.deviceObjGet(device="dut02")
        wrkston01Obj = self.topoObj.deviceObjGet(device="wrkston01")
        wrkston02Obj = self.topoObj.deviceObjGet(device="wrkston02")
