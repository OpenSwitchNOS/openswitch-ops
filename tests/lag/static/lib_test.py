...
def switchTcpdumpInterfaces(dut):
    '''
...
def deviceStopWiresharkCap(device,
    ...
    Returns:
        dict: Dictionary with parsed information
            ...
            filtered_matches (int): Number of packets matching filter argument
...
def deviceObtainActiveMacAddresses(devices):
    '''
...
def lagBroadcastTrafficTest(wrkstonSrc,
    ...
    if resWrkstons[1]['filtered_matches'] < minValue:
        ...
        LogOutput('error', 'Expected: %s - Actual: %d' %
...
def swtichGetIntoVtysh(device, enter=True):
    '''
...
def lagUnicastTrafficTest(wrkstonSrc,
    ...
    while len(linksUsed) < len(expectedUsedLinks):
        ...
        minTrafficReceived = int(ceil(trafficTransmit*(1-errTolerance)))
        if trafficReceived > maxTrafficReceived or (trafficReceived <
                                                    minTrafficReceived):
            LogOutput('error', 'Traffic received by server is different ' +
                      'than expected')
            LogOutput('error', 'Expected: Max %d bytes - Min %d bytes' %
                      (maxTrafficReceived, minTrafficReceived))
...
def lagUnicastTrafficTestVirtual(wrkstonSrc,
    ...
    while len(linksUsed) < len(expectedUsedLinks):
        ...
        if trafficReceived > maxTrafficReceived or (trafficReceived <
            ...
            LogOutput('error', 'Actual: %d bytes' % trafficReceived)
            return False
        if trafficReceived[0] != ('%.2f' % trafficTransmit):
            LogOutput('error', 'Traffic received by server is different ' +
                      'than expected')
            LogOutput('error', 'Expected: %.2f MBytes' % trafficTransmit)
            LogOutput('error', 'Actual: %s MBytes' % trafficReceived[0])
            return False
        ...
        for resSwitch, link in zip(resSwitches, dut01['lag_links'].keys()):
            if resSwitch['filtered_matches'] > 0:
                linksWithTraffic.append(link)
                if resSwitch['filtered_matches'] < int(floor(894 *
                                                             (1 -
                                                              errTolerance))):
                    LogOutput('error',
                              'Less traffic than expected was seen on' +
                              ' LAG link: Expected packets: 894 - ' +
                              'Actual packets: %d' %
        ...
        if res3['filtered_matches'] < int(floor(894 * (1 - errTolerance))):
            LogOutput(
                'error',
                'Less traffic than expected was received after client' +
                ' broadcast transmission on another workstation')
...
def lagVerifyTrafficFlow(dut01,
    ...
    wrkston01Info['original_macaddr'] = deviceObtainActiveMacAddresses(
        [wrkston01Info])[0]
    wrkston02Info = {
        'obj': wrkston02,
    ...
    wrkston02Info['original_macaddr'] = deviceObtainActiveMacAddresses(
        [wrkston02Info])[0]
    ...
    for i, dut in enumerate(duts):
        if not swtichGetIntoVtysh(dut['obj'], enter=False):
            z = i - 1
            if z >= 0:
                for k in xrange(0, z):
                    swtichGetIntoVtysh(duts[k]['obj'])
            return False
        if not switchTcpdumpInterfaces(dut):
            LogOutput('error', 'Error while obtaining interfaces ' +
                      'information for tcpdump')
            for device in duts:
                swtichGetIntoVtysh(device['obj'])
            return False
    macList = deviceObtainActiveMacAddresses(
    ...
    if not virtualTest:
        for i, dut in enumerate(duts):
            if not swtichGetIntoVtysh(dut['obj']):
                z = i + 1
                if z < len(duts):
    ...
    for wrkston, otherWrkston, dut, otherDut in zip(wrkstons,
        ...
        if not virtualTest:
            for i, device in enumerate(duts):
                if not swtichGetIntoVtysh(device['obj'], enter=False):
                    z = i - 1
                    if z >= 0:
                        for k in xrange(0, z):
                            swtichGetIntoVtysh(duts[k]['obj'])
        ...
        if not virtualTest:
            for device in duts:
                if not swtichGetIntoVtysh(device['obj']):
                    z = i + 1
                    if z < len(duts):
                        for k in xrange(z, len(duts)):
                            swtichGetIntoVtysh(duts[k]['obj'])
...
def lagCheckOwnership(dut, lagId, interfaces):
    ...
    if len(lagUuidInterfaces) != len(interfaces) and (lagUuidInterfaces[0] !=
                                                      ''):
    ...
    for dut in duts:
        ...
        try:
            ...
            if len(lagLinks) == 0:
                ...
                LogOutput('info', 'No links are present on %s and it is ' %
                          dut['lag'] + 'not possible to get its information' +
                          ' even if it is dynamic or static')
                continue
# Normal validation
            if lacpActive:
...
def switchEnableInterface(deviceObj, int, enable):
    ...
    if enable:
        ...
        if not (retStruct.returnCode() == 0 and
                interfaceGetStatus(deviceObj, int)):
            LogOutput(
