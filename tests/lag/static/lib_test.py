...
def expecting(expectHndl, command):
    ...
    '''
    expectList = ['root@\S+:.*#', 'bash-[0-9.]+#',
                  '[A-Za-z0-9]+#', pexpect.TIMEOUT]
    expectHndl.sendline(command)
    if expectHndl.expect(expectList, timeout=20) == len(expectList) - 1:
        LogOutput(
            'error',
...
def switchTcpdumpInterfaces(deviceObj):
    ...
    Returns:
        bool, dict: True and dictionary of with keys being the real interfaces
            of switch and the values what tcpdump understands. False and None
            otherwise
    '''
...
def deviceStartWiresharkCap(device, deviceLink, filter=None):
    ...
    The function does not provide context management as it can
        be called from different devices and contexts
...
def deviceStopWiresharkCap(device,
    ...
    Args:
        ...
        filter (str, optional): Filter expression to look for desired packets
            in capture
        secondaryFilter (str, optional): Second filter expression
    ...
    try:
        ...
        if secondaryFilter:
            match = re.findall(secondaryFilter, expectHndl.before)
            if match:
...
def devicesGetActiveMacAddresses(devices):
    ...
    Args:
        devices (list[dict]): Dictionary of information of interest for
            each device
...
def generateMacAddress(macList):
    ...
    random.seed()
    while True:
        mac = '%XX:%XX:%XX:%XX:%XX:%XX' % (random.randint(0, 255),
...
def generateIPAddress(ipAddrList, prefix, mask=24, maxList=254):
    '''
    Generates a new IPv4 host address to use and verifies it is not in use

    Mainly used by lagVerifyTrafficFlow

    Args:
        ipAddrList (list[str]): List of IP addresses already employed
        prefix (str): IP address prefix to be employed. This prefix is
            a network address. Eg: 10.10.0.0 from 10.10.0.0/24 network
        mask (optional, int): Mask f the prefix employed. It must be
            a value between 1 and 30. Eg. 24 from 10.10.0.0/24 network
            Default: 24
        maxList (optional, int): Max length of ip addresses already employed
            If the list length is at or more than this value, None is returned
            Default: 254

    Returns:
        str: IP address not already in use or None if no IP address could be
            generated
    '''
    prefixBin = 0
    wildMask = 0
    failList = []
    if mask > 30 or mask < 1:
        return None
    if maxList < 1:
        return None
    ...
    for i in prefixList:
        ...
        if number < 0 or number > 255:
            return None
        prefixBin = prefixBin << 8
        prefixBin += number
    wildMask = pow(2, 32 - mask) - 1
    random.seed()
    while len(failList) < wildMask - 1:
        if len(ipAddrList) >= maxList:
            return None
        val = random.randint(1, wildMask - 1)
        ipBin = val + prefixBin
        ipAddrOctets = []
        for i in xrange(0, 4):
            octet = ipBin & 255
            ipAddrOctets.insert(0, octet)
            ipBin = ipBin >> 8
        ipAddr = '{}.{}.{}.{}'.format(ipAddrOctets[0], ipAddrOctets[1],
                                      ipAddrOctets[2], ipAddrOctets[3])
        if ipAddr not in ipAddrList:
            ipAddrList.append(ipAddr)
            return ipAddr
        if ipAddr not in failList:
            failList.append(ipAddr)
...
def lagUnicastTrafficTestVirtual(wrkstonSrc,
    ...
    '''
    Code changed to reflect current behavior of LAG feature currently
    This is expected to change in the future
    '''
    if len(expectedUsedLinks) > 1:
        expectedUsedLinks = [expectedUsedLinks[0]]

    '''
    End of code change for current behavior of LAG
    '''
...
def validateEmptyConfig(devices):
    for dev in devices:
        ...
        ret_expression = re.search(
            r'Current configuration:\s*!\s*!\s*!\s*!\s*vlan 1.+no shutdown\s*(.*)\s*exit',
            output,
            re.DOTALL
        )
...
def vlanAddInterface(deviceObj, vlanId, enable, int, access=True,
                     allowed=False):
    ...
    if enable:
        ...
        retStruct = AddPortToVlan(deviceObj=deviceObj, vlanId=vlanId,
                                  interface=int, config=False, access=access,
                                  allowed=allowed)
