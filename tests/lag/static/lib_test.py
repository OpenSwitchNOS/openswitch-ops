...
def expecting(expectHndl, command):
    ...
    '''
    expectList = [r'root@\S+:.*#', r'bash-[0-9.]+#',
                  r'[A-Za-z0-9]+#', pexpect.TIMEOUT]
...
def generateMacAddress(macList):
    ...
    random.seed()
    while True:
        mac = '%.2X:%.2X:%.2X:%.2X:%.2X:%.2X' % (random.randint(0, 255),
...
def lagUnicastTrafficTest(wrkstonSrc,
    ...
    while len(linksUsed) < len(expectedUsedLinks):
        ...
        ipAddr = generateIPAddress(ipAddrList, '%s.0' % ipPrefix)
        if not ipAddr:
            LogOutput(
                'error',
...
def lagVerifyTrafficFlow(dut01,
    ...
    if not (macList or ipAddrList):
        ...
        return False
    randomUnusedIP = generateIPAddress(ipAddrList, '%s.0' % ipPrefix)
...
def validateEmptyConfig(devices):
    for dev in devices:
        ...
        ret_expression = re.search(
            r'Current configuration:.+!.+!.+!.+!\s*(.*)\s*vlan 1' +
            r'.+no shutdown.+exit',
            output,
            re.DOTALL
        )
