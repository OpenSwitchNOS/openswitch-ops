...
def lagBroadcastTrafficTest(wrkstonSrc,
    ...
    for proc2, link in zip(procs2, dut01['lag_links'].keys()):
        res2.append(deviceStopWiresharkCap(dut02['obj'],
                                           proc2,
...
def switchGetInterfaceCounters(device):
    ...
    for interface in device['lag_links'].values():
        res.append(interfaceCounters(device, interface))
    return res
...
def lagUnicastTrafficTestVirtual(wrkstonSrc,
    ...
    The function waits by default 12 seconds instead of 10 just to give
    ...
    while len(linksUsed) < len(expectedUsedLinks):
        ...
        if len(trafficReceived) != 1:
            ...
            LogOutput('error', 'Iperf client could not obtain traffic ' +
                      'report from server')
            return False
        if trafficReceived != ('%.2f' % trafficTransmit):
            LogOutput('error', 'Traffic received by server is different ' +
                      'than expected')
            LogOutput('error', 'Expected: %.2f MBytes' % trafficTransmit)
            LogOutput('error', 'Actual: %s MBytes' % trafficReceived)
            return False
        ...
        for r2, link in zip(res2, dut01['lag_links'].keys()):
            if r2['filtered_matches'] > 0:
                linksWithTraffic.append(link)
                if r2['filtered_matches'] < 894:
                    LogOutput('error',
                              'Less traffic than expected was seen on' +
                              ' LAG link: Expected packets: 894 - ' +
                              'Actual packets: %d' % r2['filtered_matches'])
                    LogOutput('error', 'Expected: At least 10')
                    LogOutput('error', 'Interface on device %s is %s' %
                              (dut01['obj'].device,
                               dut01['lag_links'][link]))
...
def lagVerifyTrafficFlow(dut01,
    ...
    #Modify unicast test if switches are virtual
    unicastTest = None
    if isinstance(dut01, VSwitch):
    ...
    if not isinstance(dut01, VSwitch):
        for i, dut in enumerate(duts):
