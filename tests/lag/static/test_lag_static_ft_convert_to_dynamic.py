...
#
###############################################################################
# Name:       test_lag_static_ft_convert_to_dynamic.py
#
...
def lagChangeMode(deviceObj, lagId, mode):
    ...
    if retStruct.returnCode() != 0:
        return False
    '''
    Commenting this section until the lacpFastFlag information source is
    fixed on VTYSH
    '''
...
#                            lacpFastFlag=retStructOriginal.\
#                            valueGet(key=str(lagId))['lacpFastFlag']):
#         return False
    if not lagVerifyConfig(deviceObj, str(lagId),
                           interfaces=retStructOriginal.\
                           valueGet(key=str(lagId))['interfaces'],
...
class Test_ft_convert_to_dynamic:

    def setup_class(cls):
        # Create Topology object and connect to devices
        Test_ft_convert_to_dynamic.testObj =\
            testEnviron(topoDict=topoDict)
        Test_ft_convert_to_dynamic.topoObj =\
            Test_ft_convert_to_dynamic.testObj.topoObjGet()
    ...
    def teardown_class(cls):
        ...
        # Terminate all nodes
        Test_ft_convert_to_dynamic.topoObj.terminate_nodes()
    ...
    def test_enableDUTsInterfaces(self):
        ...
        for dut in [dut01, dut02]:
            LogOutput('info', "Configuring switch %s" % dut['obj'].device)
            for link in dut['links']:
                asisto(switchEnableInterface(dut['obj'],
    ...
    def test_createLAGs(self):
        ...
        dut01Obj = self.topoObj.deviceObjGet(device="dut01")
        dut02Obj = self.topoObj.deviceObjGet(device="dut02")
        asisto(lagCreate(dut01Obj, '1', True, [
            dut01Obj.linkPortMapping['lnk02'],
            dut01Obj.linkPortMapping['lnk03']], 'off'))
        asisto(lagCreate(dut02Obj, '1', True, [
            dut02Obj.linkPortMapping['lnk02'],
            dut02Obj.linkPortMapping['lnk03']], 'off'))
        asisto(lagVerifyNumber(dut01Obj, 1))
        asisto(lagVerifyNumber(dut02Obj, 1))
    ...
    def test_configureVLANs(self):
        ...
        for dut in [dut01, dut02]:
            ...
            asisto(vlanConfigure(dut['obj'], 900, True))
            for link in dut['links']:
                asisto(vlanAddInterface(dut['obj'], 900, True, link))
    ...
    def test_LAGFormation1(self):
        ...
        wrkston01Obj = self.topoObj.deviceObjGet(device="wrkston01")
        wrkston02Obj = self.topoObj.deviceObjGet(device="wrkston02")
        asisto(lagVerifyLACPStatusAndFlags(dut01Obj, dut02Obj, False,
                                           ['lnk02', 'lnk03'], 'lag1',
                                           'lag1'))
        asisto(lagVerifyTrafficFlow(dut01Obj, dut02Obj, wrkston01Obj,
    ...
    def test_lagChangeMode(self):
        LogOutput('info', "\n############################################")
        LogOutput('info', "Change LAGs from static to dynamic")
        LogOutput('info', "############################################")
        dut01Obj = self.topoObj.deviceObjGet(device="dut01")
        dut02Obj = self.topoObj.deviceObjGet(device="dut02")
        LogOutput('info', "Change LAG mode on dut01")
        asisto(lagChangeMode(dut01Obj, '1', 'active'))
        LogOutput('info', "Change LAG mode on dut02")
        asisto(lagChangeMode(dut02Obj, '1', 'passive'))
    ...
    def test_LAGFormation2(self):
        ...
        wrkston01Obj = self.topoObj.deviceObjGet(device="wrkston01")
        wrkston02Obj = self.topoObj.deviceObjGet(device="wrkston02")
        asisto(lagVerifyLACPStatusAndFlags(dut01Obj, dut02Obj, True,
        ...
        for dut in [dut01, dut02]:
            ...
            for link in dut['links']:
                asisto(switchEnableInterface(dut['obj'],
                                             dut['obj'].linkPortMapping[link],
                                             True))
        LogOutput('info', "Resting 2 seconds to continue test...")
        sleep(2)
