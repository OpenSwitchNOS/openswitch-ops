...
4. Associate interfaces 1 and 2 to the LAG from both switches
5. Wait 30 seconds for switches to negotiate LAG state
6. Change lacp mode in port from "active" to "off" on s2
7. Verify that interfaces on switch 1 are out of sync
8. LACP mode back to active on s2
9. Override port parameters on s1 and s2 (set other_config lacp-fallback-ab as true)
...
13. LACP mode back to active on s2
