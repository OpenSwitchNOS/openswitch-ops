...
9. Override port parameters on switch 1 and switch 2 (set other_config lacp-fallback-ab as true or lacp fallback on cli)
10. Change LACP mode in port from "active" to "off" on switch 2
11. Get LAG information for interface 1 in switch 1
12. Verify that one interface are in state up for switch 1
...
* Any interface is up when fallback is true, or more than one is up.
* Some interface is up when fallback is false and LAG is not active.
