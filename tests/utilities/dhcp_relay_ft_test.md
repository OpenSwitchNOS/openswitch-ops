...
6. Verify that the DHCP Client has not received an IPv4 address.

### Test results criteria
#### Test pass criteria
This test succeeds if the DHCP client does not recieve an IPv4 address.
#### Test fail criteria
This test fails if the DHCP client recieves an IPv4 address.
...
5. Verify that the DHCP Client has received an IPv4 address.
6. Remove server IP address as helper addresses on DHCP relay agent.
7. Request DHCP address on client.
8. Verify that the DHCP Client has not received an IPv4 address.
...
4. Do not add option 82 info on original request.
...
6. Use ixia or other means to send DHCP server responses packets to DHCP relay agent with:
    ...
    - No option 82
7. Verify using sniffer that server response packets are dropped on DHCP relay agent. DHCP relay agent is dropping them (drop counter should reflect this).
...
9. Verify using sniffer that server response packets are dropped on DHCP relay agent. DHCP relay agent is dropping them (drop counter should reflect this).
10. Use ixia or other means to send DHCP server responses packets to DHCP relay agent with:
...
12. Use ixia or other means to send DHCP server responses packets to DHCP relay agent with:
    ...
    - Circuit ID = DHCP relay agent curcuit ID
    - Remote ID = DHCP relay agent remote ID
13. Verify using sniffer that server response packets are forwarded to interface. DHCP relay agent counter should reflect this.
