...
def ping_with_options(**kwargs):
    ...
    #Ping IPv4-address from switch1 to switch3 with the data-fill parameter
    ...
    retCode = devIntRetStruct.get('returnCode')
    assert retCode == 0, "Failed to ping IPv4-address"
    " from switch1 to switch3 with the data-fill parameter"

    retBuffer = devIntRetStruct.get('buffer')
    assert 'PING 10.0.40.1 (10.0.40.1): 100 data bytes' in retBuffer, "Ping"
    " IPv4-address from switch1 to switch3 with the data-fill parameter failed"

    #Ping IPv4-address from switch1 to switch3 with the datagram-size parameter
    LogOutput('info', "Test ping IPv4-address from switch1 to switch3"
    ...
    retCode = devIntRetStruct.get('returnCode')
    assert retCode == 0, "Failed to ping IPv4-address"
    " from switch1 to switch3 with the datagram-size parameter"

    retBuffer = devIntRetStruct.get('buffer')
    assert 'PING 10.0.40.1 (10.0.40.1): 200 data bytes' in retBuffer, "Ping"
    " IPv4-address from switch1 to switch3"
    " with the datagram-size parameter failed"

    #Ping IPv4-address from switch1 to switch3 with the interval parameter
    LogOutput('info', "Test ping IPv4-address from switch1 to switch3"
    ...
    retCode = devIntRetStruct.get('returnCode')
    assert retCode == 0, "Failed to ping IPv4-address"
    " from switch1 to switch3 with the interval parameter"

    retBuffer = devIntRetStruct.get('buffer')
    assert 'PING 10.0.40.1 (10.0.40.1): 100 data bytes' in retBuffer, "Ping"
    " IPv4-address from switch1 to switch3 with the interval parameter failed"

    #Ping IPv4-address with the repetition parameter from switch1 to switch3
    LogOutput('info', "Test ping IPv4-address from switch1 to switch3"
    ...
    retCode = devIntRetStruct.get('returnCode')
    assert retCode == 0, "Failed to ping IPv4-address"
    " from switch1 to switch3 with the repetition parameter"
    ...
    #Ping IPv4-address with the timeout parameter from switch1 to switch3
    LogOutput('info', "Test ping IPv4-address from switch1 to switch3"
    ...
    retCode = devIntRetStruct.get('returnCode')
    assert retCode == 0, "Failed to ping IPv4-address"
    " from switch1 to switch3 with the timeout parameter"

    retBuffer = devIntRetStruct.get('buffer')
    assert 'PING 10.0.40.1 (10.0.40.1): 100 data bytes' in retBuffer, "Ping"
    " IPv4-address from switch1 to switch3 with the timeout parameter failed"

    #Ping IPv4-address with the TOS parameter from switch1 to switch3
    LogOutput('info', "Test ping IPv4-address from switch1 to switch3"
    ...
    retCode = devIntRetStruct.get('returnCode')
    assert retCode == 0, "Failed to ping IPv4-address"
    " from switch1 to switch3 with the TOS parameter"

    retBuffer = devIntRetStruct.get('buffer')
    assert 'PING 10.0.40.1 (10.0.40.1): 100 data bytes' in retBuffer, "Ping"
    " IPv4-address from switch1 to switch3 with the TOS parameter failed"
...
def ping_extended_options(**kwargs):
    ...
    retCode = devIntRetStruct.get('returnCode')
    assert retCode == 0, "Failed to ping IPv4-address"
    " from switch1 to switch3 with the ip-option record-route"

    retBuffer = devIntRetStruct.get('buffer')
    assert 'RR:' in retBuffer, "Ping from switch1"
    " to switch3 with the ip-option record-route failed"
    ...
    retCode = devIntRetStruct.get('returnCode')
    assert retCode == 0, "Failed to ping IPv4-address"
    " from switch1 to switch3 with the ip-option include-timestamp"

    retBuffer = devIntRetStruct.get('buffer')
    assert 'TS:' in retBuffer, "Ping from switch1 to switch3"
    ...
    retCode = devIntRetStruct.get('returnCode')
    assert retCode == 0, "Failed to ping IPv4-address"
    " from switch1 to switch3 with the ip-option include-timestamp-and-address"

    retBuffer = devIntRetStruct.get('buffer')
    assert 'TS:' in retBuffer, "Ping from"
    " switch1 to switch3 with the ip-option"
...
def ping6_with_options(**kwargs):
    ...
    #Ping6 from switch3 to switch1 with the data-fill parameter
    LogOutput('info', "Test ping6 IPv6-address from switch3 to switch1"
              " with the data-fill parameter")
    devIntRetStruct = switch3.DeviceInteract(command="ping6 1030::1"
                                             " data-fill dee")
    retCode = devIntRetStruct.get('returnCode')
    assert retCode == 0, "Failed to ping6 IPv6-address"
    " with the data-fill parameter from switch3 to switch1"

    retBuffer = devIntRetStruct.get('buffer')
    assert 'PING 1030::1 (1030::1): 100 data bytes' in retBuffer, "Ping6"
    " from switch3 to switch1 with the data-fill parameter failed"

    #Ping6 from switch3 to switch1 with the datagram-size parameter
    LogOutput('info', "Test ping6 IPv6-address from switch3 to switch1"
              " with the datagram-size parameter")
    devIntRetStruct = switch3.DeviceInteract(command="ping6 1030::1"
                                             " datagram-size 200")
    retCode = devIntRetStruct.get('returnCode')
    assert retCode == 0, "Failed to ping6 IPv6-address"
    " from switch3 to switch1 with the datagram-size parameter"

    retBuffer = devIntRetStruct.get('buffer')
    assert 'PING 1030::1 (1030::1): 200 data bytes' in retBuffer, "Ping6"
    " from switch1 to switch1 with the datagram-size parameter failed"

    #Ping6 from switch3 to switch1 with the interval parameter
    LogOutput('info', "Test ping6 IPv6-address from switch3 to switch1"
              " with the interval parameter")
    devIntRetStruct = switch3.DeviceInteract(command="ping6 1030::1"
                                             " interval 2")
    retCode = devIntRetStruct.get('returnCode')
    assert retCode == 0, "Failed to ping6 IPv6-address"
    " from switch3 to switch1 with the interval parameter"

    retBuffer = devIntRetStruct.get('buffer')
    assert 'PING 1030::1 (1030::1): 100 data bytes' in retBuffer, "Ping6"
    " from switch3 to switch1 with the interval parameter failed"

    #Ping6 from switch3 to switch1 with the repetition parameter
    ...
    retCode = devIntRetStruct.get('returnCode')
    assert retCode == 0, "Failed to ping6 IPv6-address"
    " from switch3 to switch1 with the repetition parameter"

    retBuffer = devIntRetStruct.get('buffer')
    assert 'PING 1030::1 (1030::1): 100 data bytes' in retBuffer, "Ping6"
    " from switch3 to switch1 with the repetition parameter failed"


def ping_failure_cases(**kwargs):
    ...
    devIntRetStruct = switch2.DeviceInteract(command="ping 10.0.40.5")
    retCode = devIntRetStruct.get('returnCode')
    assert retCode == 0, "Failed to ping IPv4-address"
...
def ping6_failure_cases(**kwargs):
...
class Test_ping:
    ...
    def test_ping_full(self):
        ...
        ping_failure_cases(switch1=dut01Obj, switch2=dut02Obj)

        LogOutput('info', "\n### Ping6 failure test cases ###")
        ping6_failure_cases(switch1=dut01Obj, switch2=dut02Obj)
