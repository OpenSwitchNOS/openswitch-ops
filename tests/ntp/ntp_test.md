...
Please use the command below on the switch to restart the ops-ntpd service.

Following command is not a vtysh cli command and should be run on the linux shell.
To come out of the vtysh, use `exit`on the vtysh and then execute following command.
...
Restart the ops-ntpd service on the switch and check if NTP associations and NTP status is verified
Please use the command below to restart the ops-ntpd service
