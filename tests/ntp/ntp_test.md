...
### Objective
The test case checks if the switch is configured as a Network Time Protocol (NTP) client and if the switch is successfully configured with local NTP server.
...
```
  ...
  ==============================================================================
  *15.1.1.4           .GPS.       2 u    9   64    1   48.865  -51.213   0.000
...
Restart the ops-ntpd service on the switch and check if NTP associations and NTP status is verified
Please use the following command on the switch to restart ops-ntpd service

 ```
 service restart ops-ntpd
 ```

After restarting ops-ntpd service, make sure that the NTP association and NTP status is verified.
...
2. One more way to confirm the server configuration is to check the ntp status and check if the clock is synchronized
with the ntp server

3. We also make sure that the local server can reach the public server it has configured with.
If its not then NTP client can not be configured with local NTP server

4. We have added the non authenticating server as preferred server. We have to make sure that this perticular server IP is synchronized with the client.
Example output:
 ...
 ----------------------------------------------------------------------------------------------------------------------
    1       172.17.0.2       172.17.0.2    3     55           .GPS.    2  U     4    64     17    0.109  -0.005   0.040
 *  2       172.17.0.3       172.17.0.3    3      -           .GPS.    2  U     4    64     17    0.133   0.023   0.032
 ----------------------------------------------------------------------------------------------------------------------
...
Restart the ops-ntpd service on the switch and check if NTP associations and NTP status is verified
Please use the following command to restart ops-ntpd service
...
4. We have added the authenticating server as preferred server. We have to make sure that this perticular server IP is synchronized with the client.
...
----------------------------------------------------------------------------------------------------------------------
  ID             NAME           REMOTE  VER  KEYID           REF-ID  ST  T  LAST  POLL  REACH    DELAY  OFFSET  JITTER
----------------------------------------------------------------------------------------------------------------------
* 1       172.17.0.2       172.17.0.2    3     55            .GPS.   2  -     -    64      0    0.000   0.000   0.000
  2       172.17.0.3       172.17.0.3    3      -            .GPS.   2  -    66    64      7    0.126   0.029   0.016
----------------------------------------------------------------------------------------------------------------------
