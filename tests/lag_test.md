...
then by a L3 LAG and finally connected by a L2 LAG again.
  ...
  4. Configure LAGs and workstations interfaces with same VLAN.
  5. Ping from workstation 1 to workstation 2 and viceversa.
  6. Configure LAGs as "routing" and assign an IP address on the same range to
     each LAG .
  7. Ping from switch 1 to switch 2 and viceversa and
     from workstation 1 to workstation 2 and viceversa.
  8. Configure LAGs as "no routing" and configure LAGs and workstations
...
from a L2 LAG to a L3 LAG and viceversa.
  ...
  5. Configure L3 LAGs as "routing" and assign an IP address on the same range to
     each L3 LAG.
...
### Test result criteria
#### Test pass criteria
 * The ping between the workstations succeeds when interface 2 is part of L2 LAG
   and fails when interface 2 is part of L3 LAG.
 * The ping between the switches succeeds when interface 2 is part of L3 LAG
   and fails when interface 2 is part of L2 LAG.

#### Test fail criteria
 * The ping between the workstations fails when interface 2 is part of L2 LAG
   or succeeds when interface 2 is part of L3 LAG.
 * The ping between the switches fails when interface 2 is part of L3 LAG
   or succeeds when interface 2 is part of L2 LAG.
