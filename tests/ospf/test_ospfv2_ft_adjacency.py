...
class Test_ospf_configuration:
    ...
    # [1.05] - Latest router-id  configured should be retained
    # when instance level router-id is removed
    def test_dynamic_router_selection(self):
        LogOutput('info', "*****[1.05] Latest router-id  configured should be"
        ...
        LogOutput('info', "Step 3 - verify static router-id"
                  "is removed using show ip ospf command")
        router_id = get_router_id(dut01Obj)
        if OSPF_ROUTER_ID_DUT1 in router_id:
            assert False, "Static router-id not removed"
        else:
            LogOutput('info', "Static router-id removed")
    ...
    def test_neighbors_are_discovered(self):
    ...
    # [2.04] -Test case to verify that neighbor adjacency
    # is torn when one of the neighbor goes down
    def test_adjacency_by_disabling_interface(self):

        LogOutput('info', "******[2.04] Verifying that neighbor adjacency "
                  "is torn when one of the neighbor goes down*****")
    ...
    # [2.03]Test case to verify that neighbor adjacency
    # is torn when ospfv2 is disabled in one of the switches
    def test_adjacency_by_deleting_instance(self):
        LogOutput('info', "***** [2.03] Verifying that neighbor adjacency"
    ...
    # [2.07] - Test case to verify adjacency
    # is torn when MTU mismatches
    def test_verify_mtu_mismatch(self):
        LogOutput('info', "***** [2.07] Test case to verify that neighbor"
                  " adjacency is torn when MTU mismatches")
