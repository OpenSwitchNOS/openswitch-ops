...
def verify_ospf_adjacency(dut01, dut02_router_id, print_nbrs=False):
        neighbors = SwitchVtyshUtils.vtysh_cmd(dut01, "sh ip ospf neigh")

        if print_nbrs:
            #info("### Neighbors for switch %s ###\n" % dut01.name)
            info("%s\n" % neighbors)
...
def configure(**kwargs):
    ...
    '''
    - Enable the link.
    - Set IP for all the four switches.
    '''
    ...
    '''
    createRouterInstanceTest(switch1)
    ...
    createRouterInstanceTest(switch2)
