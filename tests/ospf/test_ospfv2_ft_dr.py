...
def configure_mtu(dut01, interface, enable, mtu_value):
...
def getTcpdump(switch_id):
    ...
    devIntReturn = switch_id.DeviceInteract(command="ifconfig")
    retCode = devIntReturn.get('returnCode')
    assert retCode == 0, "Failed to execute command"
    matchObj = re.search(r'MTU:\d{1,4}', devIntReturn['buffer'], re.S)
    if matchObj:
...
def wait_for_2way_state(dut):
    ...
    for i in range(wait_time):
        exstart_state = verify_ospf_adjacency(dut, "Exstart")
        exchange_state = verify_ospf_adjacency(dut, "Exchange")
        loading_state = verify_ospf_adjacency(dut, "Loading")
        full_state = verify_ospf_adjacency(dut, "Full")

        if (exstart_state or exchange_state or loading_state or full_state):
            return True
        else:
            sleep(1)
...
class Test_ospf_configuration:
    ...
    def test_dynamic_router_selection(self):
        LogOutput('info', "*****[1.05] Verifying that the dynamic router id is"
                  " selected when instance level router id is removed *****")
        ...
        router_id = get_router_id(dut01Obj)
        if OSPF_ROUTER_ID_DUT1 in router_id:
            LogOutput('info', "Instance level Router-id not removed")
            assert False, "Instance level Router-id not removed"
        else:
            LogOutput('info', "Instance level Router-id removed")
        ...
        LogOutput('info', "Step 5 - verify router ID"
                  " through hellp packet in tcp-dump")
        router_id = get_router_id(dut01Obj)
        if (neighbor_id == OSPF_ROUTER_ID_DUT1):
            LogOutput('info', "Router-id found in hello packet")
        else:
            LogOutput('info', "Router-id found seen in hello packet")
            assert False, "Router-id not found in hello packet"

        LogOutput('info', "******[1.05] Passed ******")
    ...
    # [2.07] - Test case to verify that neighbor
    # information is updated when MTU mismatches
    def test_verify_mtu(self):
        LogOutput('info', "***** [2.07] Test case to verify that neighbor"
                  " information is updated when MTU mismatches")
        ...
        interface_value = switch.linkPortMapping[str(link)]
        retVal = configure_mtu(switch, interface_value, True, "1200")
        if retVal:
            LogOutput('info', "Step 4 - verifying MTU")
            retVal = verify_mtu(switch, interface_value, 1200)
            if retVal:
