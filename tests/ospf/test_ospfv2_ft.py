   ...
   # Enabling interface
    ...
    if "OSPF_LINK_KEY" in dict:
       ...
       retStruct = InterfaceEnable(deviceObj=switch, enable=True,
                                interface=switch.linkPortMapping[str(link)])
       retCode = retStruct.returnCode()
       if retCode != 0:
...
class Test_ospf_configuration:
    ...
    #[1.04] - Verifying that the router id configured in router context is selected
    def test_nbr_discovery(self):

        print "Verifying that the router id configured in router context is selected"
   ...
   #is updated when ospfv2 is disabled in one of the switches
    ...
    #[1.05] - Verifying that the dynamic router id is selected when instance level router id is removed
    ...
    #[2.04] -Test case to verify that neighbor information is updated when one of the neighbor goes down
   ...
   #[3.04] :Test case to verify that the DR election is triggered when router ids are changed.
