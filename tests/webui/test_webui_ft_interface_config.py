...
ADMIN_PATCH_PORT = [{"op": "add", "path": "/admin", "value": "up"}]
ADMIN_PATCH_INT = [{"op": "add", "path": "/user_config", "value": {"admin": "up"}}]
...
class Test_CreatePatch(OpsVsiTest):
    ...
    def test_create_port(self):
        ...
        status_code, response_data = execute_request(self.PATH_PORTS, "POST", json.dumps(BASIC_PORT_DATA), self.SWITCH_IP)
        ...
        assert json_data["configuration"] == BASIC_PORT_DATA["configuration"], "Configuration data is not equal that posted data"
    ...
    def test_patch_port_int_admin(self):
        ...
        basic_port_data["configuration"]["interfaces"] = ["/rest/v1/system/interfaces/2"]
        ...
        status_code, response_data = execute_request(self.PORT_PATH, "PATCH", json.dumps(ADMIN_PATCH_PORT), self.SWITCH_IP, False)
        assert status_code == httplib.NO_CONTENT, "Error patching a Port. Status code: %s Response data: %s " % (status_code, response_data)
        ...
        status_code, response_data = execute_request(self.INT_PATH, "PATCH", json.dumps(ADMIN_PATCH_INT), self.SWITCH_IP, False)
        assert status_code == httplib.NO_CONTENT, "Error patching an Interface. Status code: %s Response data: %s " % (status_code, response_data)
        info("### Interface Patched. Status code is 204 NO CONTENT  ###\n")

        # Verify data
        status_code, response_data = execute_request(self.INT_PATH, "GET", None, self.SWITCH_IP)
        assert status_code == httplib.OK, "Failed to query patched Port"
        json_data = {}
        try:
            json_data = json.loads(response_data)
            ...
            assert False, "Malformed JSON"
        assert json_data["configuration"] == basic_int_data["configuration"], "Configuration data is not equal that posted data"
        info("### Configuration data validated ###\n")


        info("\n########## End Test to Validate Create And Patch Port Int ##########\n")
