...
def lldp_interface_txrx(**kwargs):
    ...
    for retry in range(1,3):
        ...
        if str(lnk01PrtStats[device1.linkPortMapping['lnk01']]['Neighbor_portID']).isdigit() is True:
            break
        else:
            ...
            neighbor_output = device1.cmd(command="ovs-vsctl list interface "
                                          + device1.linkPortMapping['lnk01']
    ...
    for retry in range(1,3):
        ...
        if str(lnk01PrtStats[device2.linkPortMapping['lnk01']]['Neighbor_portID']).isdigit() is True:
            break
        else:
            # Dump out the ovs-vsctl interface information
            LogOutput('info', "Didn't receive integer value for Neightbor_portID, dumping ovs-vsctl interface stats...")
            neighbor_output = device2.cmd(command="ovs-vsctl list interface "
                                          + device2.linkPortMapping['lnk01']
                                          + " | grep lldp_neighbor_info")
            opstestfw.LogOutput('info', "ovs-vsctl list interface output:\n" + neighbor_output)
            Sleep(seconds=10, message="Delay")
    ...
    for retry in range(1,3):
        ...
        if str(lnk02PrtStats[device1.linkPortMapping['lnk02']]['Neighbor_portID']).isdigit() is True:
            break
        else:
            # Dump out the ovs-vsctl interface information
            LogOutput('info', "Didn't receive integer value for Neightbor_portID, dumping ovs-vsctl interface stats...")
            neighbor_output = device1.cmd(command="ovs-vsctl list interface "
                                          + device1.linkPortMapping['lnk02']
                                          + " | grep lldp_neighbor_info")
            opstestfw.LogOutput('info', "ovs-vsctl list interface output:\n" + neighbor_output)
            Sleep(seconds=10, message="Delay")
    device1.setDefaultContext(context="vtyShell")
    ...
    for retry in range(1,3):
        ...
        if str(lnk02PrtStats[device2.linkPortMapping['lnk02']]['Neighbor_portID']) == "" :
            break
        else:
            # Dump out the ovs-vsctl interface information
            LogOutput('info', "Didn't receive integer value for Neightbor_portID, dumping ovs-vsctl interface stats...")
            neighbor_output = device2.cmd(command="ovs-vsctl list interface "
                                          + device2.linkPortMapping['lnk02']
                                          + " | grep lldp_neighbor_info")
            opstestfw.LogOutput('info', "ovs-vsctl list interface output:\n" + neighbor_output)
            Sleep(seconds=10, message="Delay")
    device2.setDefaultContext(context="vtyShell")
    ...
    for retry in range(1,3):
        ...
        if str(lnk03PrtStats[device1.linkPortMapping['lnk03']]['Neighbor_portID']) == "":
            break
        else:
            # Dump out the ovs-vsctl interface information
            LogOutput('info', "Didn't receive integer value for Neightbor_portID, dumping ovs-vsctl interface stats...")
            neighbor_output = device1.cmd(command="ovs-vsctl list interface "
                                          + device1.linkPortMapping['lnk03']
                                          + " | grep lldp_neighbor_info")
            opstestfw.LogOutput('info', "ovs-vsctl list interface output:\n" + neighbor_output)
            Sleep(seconds=10, message="Delay")
    device1.setDefaultContext(context="vtyShell")
    ...
    for retry in range(1,3):
        ...
        if str(lnk03PrtStats[device2.linkPortMapping['lnk03']]['Neighbor_portID']).isdigit() is True:
            break
        else:
            # Dump out the ovs-vsctl interface information
            LogOutput('info', "Didn't receive integer value for Neightbor_portID, dumping ovs-vsctl interface stats...")
            neighbor_output = device2.cmd(command="ovs-vsctl list interface "
                                          + device2.linkPortMapping['lnk03']
                                          + " | grep lldp_neighbor_info")
            opstestfw.LogOutput('info', "ovs-vsctl list interface output:\n" + neighbor_output)
            Sleep(seconds=10, message="Delay")
    device2.setDefaultContext(context="vtyShell")
    ...
    for retry in range(1,3):
        ...
        if str(lnk04PrtStats[device1.linkPortMapping['lnk04']]['Neighbor_portID']) == "":
            break;
        else:
            # Dump out the ovs-vsctl interface information
            LogOutput('info', "Didn't receive integer value for Neightbor_portID, dumping ovs-vsctl interface stats...")
            neighbor_output = device1.cmd(command="ovs-vsctl list interface "
                                          + device1.linkPortMapping['lnk04']
                                          + " | grep lldp_neighbor_info")
            opstestfw.LogOutput('info', "ovs-vsctl list interface output:\n" + neighbor_output)
            Sleep(seconds=10, message="Delay")
    device1.setDefaultContext(context="vtyShell")
    ...
    for retry in range(1,3):
        ...
        if str(lnk04PrtStats[device2.linkPortMapping['lnk04']]['Neighbor_portID']) == "":
            break
        else:
            # Dump out the ovs-vsctl interface information
            LogOutput('info', "Didn't receive integer value for Neightbor_portID, dumping ovs-vsctl interface stats...")
            neighbor_output = device2.cmd(command="ovs-vsctl list interface "
                                          + device2.linkPortMapping['lnk04']
                                          + " | grep lldp_neighbor_info")
            opstestfw.LogOutput('info', "ovs-vsctl list interface output:\n" + neighbor_output)
            Sleep(seconds=10, message="Delay")
    device2.setDefaultContext(context="vtyShell")
