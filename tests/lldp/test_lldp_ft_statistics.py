...
def lldp_statistics(**kwargs):
    ...
    #Showing the transmitted and received packets ,must be either 6 or 7
    ...
    LogOutput('info', "\nTotal Packets Received: "+str(Global['Total_Packets_Received']))
    LogOutput('info', "\nTotal Packets Transmitted: "+str(Global['Total_Packets_Transmitted']))
    assert (int(Global['Total_Packets_Received'])==6 or int(Global['Total_Packets_Received'])==7) , "\nCase for packets received failed"
    assert (int(Global['Total_Packets_Transmitted'])==6 or int(Global['Total_Packets_Transmitted'])==7) , "\nCase for packets transmitted failed"
...
class Test_lldp_configuration:
    ...
    def test_lldp_statistics(self):
        ...
        retValue = lldp_statistics(device1=dut01Obj, device2=dut02Obj)
