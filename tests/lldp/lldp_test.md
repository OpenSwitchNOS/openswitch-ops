...
3. Configure the schema equivalent of the above OID through the CLI. Query for an OID again.
...
#### Test case 2
##### Objective
This test verifies that a snmpget(v1/v2/v3) query for a LLDP object which does not have a schema equivalent, results in returning the default value specified in the convert function. The testcase is verified from a localhost and from workstation as well.
...
##### Pass/Fail criteria
- This test passes if the query returns the default value specified in the convert function.
- This test fails if the query does not return the default value specified in the convert function.
...
#### Test case 2
##### Objective
This test verifies that a snmpgetNext(v1/v2/v3) query for a LLDP object which does not have a schema equivalent, results in a returning the default value specified in the convert function.The testcase is verified from a localhost and from workstation as well.
...
##### Pass/Fail criteria
- The test passes if the query returns the default value specified in the convert function.
- The test fails if the query does not return the default value specified in the convert function.
...
1. Create a topology with a single switch, switch 1 and a workstation WS.
2. Query for the OID.
3. Configure the schema equivalent of the above OID through the CLI. Query the OID again.
...
##### Pass/Fail criteria
- The test passes if the query walks through all the OIDs.
- This tests fails if the query does not walk through all the OIDs.
