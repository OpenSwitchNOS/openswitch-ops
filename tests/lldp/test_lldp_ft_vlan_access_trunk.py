...
from opstestfw import returnStruct
...
def lldp_vlan_access(**kwargs):
    ...
    if myretStruct.returnCode() == 0:
        ...
        print "#######"
        assert (str(PacketDict[1]["VlanNameTLV"][0]).lower() == "vlan10" and
...
def lldp_vlan_trunk(**kwargs):
    ...
    if myretStruct.returnCode() == 0:
        ...
        # Details of the first frame
        assert (str(PacketDict[1]["VlanNameTLV"][0]).lower() == "vlan10" and
                str(PacketDict[1]["VlanNameTLV"][1]).lower() == "vlan20" and
                str(PacketDict[1]["VlanNameTLV"][2]).lower() == "vlan30" and
                str(PacketDict[1]["VlanNameTLV"][3]).lower() == "vlan40" and
                str(PacketDict[1]["PortVLanID"]) == "10 (0x000A)"), \
            "\n**Case Failed**\n"
        LogOutput('info', "\n**Case Passed**\n")
    else:
...
class Test_lldp_vlan:
    ...
    def test_lldp_vlan_access(self):
        ...
        wrkston01Obj.commandErroCheck = 0
    ...
    def test_lldp_vlan_trunk(self):
        ...
        wrkston02Obj.commandErroCheck = 1
