...
topoDict = {"topoExecution": 1000,
            ...
            "topoFilters": "dut01:system-category:switch,wrkston01:system-category:workstation,wrkston01:docker-image:host/freeradius-ubuntu,wrkston02:system-category:workstation,wrkston02:docker-image:host/freeradius-ubuntu"}

def lldp_vlan_access(**kwargs):
    device1 = kwargs.get('device1',None)
    host = kwargs.get('device2',None)

    LogOutput('info',"Test 1 : Access Vlan")
    ...
    AddPortToVlan(deviceObj=device1, vlanId=10, interface=device1.linkPortMapping['lnk01'], access=True)
    ...
    CaptureObj = opstestfw.host.PacketCapture(host.device,host.linkPortMapping['lnk01'],"Capture.log")
    ...
    if myretStruct.returnCode() == 0:
        ...
        PacketDict = dict()
        PacketDict = myretStruct.valueGet(key=1)
        LogOutput('info',""+str(PacketDict))
        assert (str(PacketDict["VlanName"])=="vlan10" and str(PacketDict["PortVLanID"])=="10 (0x000A)"), "\n**Case Failed**\n"
        LogOutput('info',"\n**Case Passed**\n")
...
def lldp_vlan_trunk(**kwargs):
    ...
    AddPortToVlan(deviceObj=device1, vlanId=10, interface=device1.linkPortMapping['lnk02'])
    AddPortToVlan(deviceObj=device1, vlanId=20, interface=device1.linkPortMapping['lnk02'], allowed=True)
    AddPortToVlan(deviceObj=device1, vlanId=30, interface=device1.linkPortMapping['lnk02'], allowed=True)
    AddPortToVlan(deviceObj=device1, vlanId=40, interface=device1.linkPortMapping['lnk02'], allowed=True)
    ...
    CaptureObj = opstestfw.host.PacketCapture(host.device,host.linkPortMapping['lnk02'],"Capture.log")
    retStruct = CaptureObj.StartCapture(connection = host)
    Sleep(seconds=40, message="Waiting for LLDP packets to be captured")
    myretStruct = CaptureObj.ParseCapture(host)
    if myretStruct.returnCode() == 0:
        ...
        PacketDict = myretStruct.valueGet(key=1)
        LogOutput('info',""+str(PacketDict))
        assert (str(PacketDict["VlanName"])=="vlan40" and str(PacketDict["PortVLanID"])=="10 (0x000A)"), "\n**Case Failed**\n"
        LogOutput('info',"\n**Case Passed**\n")
...
class Test_lldp_vlan:
    ...
    def test_lldp_vlan_access(self):
        ...
        retValue = lldp_vlan_access(device1=dut01Obj, device2=wrkston01Obj)
        LogOutput('info',"Test 2 : Trunk Vlan")

    def test_lldp_vlan_trunk(self):
        dut01Obj = self.topoObj.deviceObjGet(device="dut01")
        wrkston02Obj = self.topoObj.deviceObjGet(device="wrkston02")
        retValue = lldp_vlan_trunk(device1=dut01Obj, device2=wrkston02Obj)
