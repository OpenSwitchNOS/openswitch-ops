...
class Test_ft_LAG_static_L2_VLAN_Hashing_Flow_Distribution:
    ...
    def test_interface_vlan(self):
        ...
        # Configured Vlan for device 3
        devIntLagRetStruct = AddPortToVlan(
                deviceObj=dut03Obj,
                vlanId=500,
                interface=dut03Interface01,
                access=True,
                config=True)

        if devIntLagRetStruct.returnCode() != 0:
            LogOutput('error',
                      "Failed to configured vlan 500 in the interface")
            assert(False)
        else:
            LogOutput('info', "Passed interface vlan 500 configured")

        devIntLagRetStruct = AddPortToVlan(
                deviceObj=dut03Obj,
                vlanId=600,
                interface=dut03Interface02,
                access=True,
                config=True)

        if devIntLagRetStruct.returnCode() != 0:
            LogOutput('error',
                      "Failed to configured vlan 600 in the interface")
            assert(False)
        else:
            LogOutput('info', "Passed interface vlan 600 configured")
    ...
    def test_configured_switch_ip_address(self):
        ...
        if retStruct.returnCode() != 0:
            LogOutput('error', "Failed to configured  IP on vlan 500")
            assert(False)
        else:
            LogOutput('info', "Successfully configured IP on vlan 500")
        ...
        if retStruct.returnCode() != 0:
            LogOutput('error', "Failed to configured  IP on vlan 600")
            assert(False)
        else:
            LogOutput('info', "Successfully configured IP on vlan 600")
    ...
    def test_send_validated_traffic(self):
        ...
        raw1Tx = (int(tx1Second) - int(tx1Firts)) - \
            (int(tx2Second) - int(tx2Firts))
