...
def addInterfaceToLAG(deviceObj, lagId, int, config, expectedIntArray):
    ...
    if returnStruct.returnCode() != 0:
        ...
        return False
    if len(returnStruct.valueGet(key=lagId)['interfaces']) != len(expectedIntArray):
        LogOutput(
            'error',
...
class Test_ft_framework_basics:
    ...
    def test_clean_up_devices(self):
        ...
        finalResult.append(configureWorkstation(wrkston01Obj, wrkston01Obj.linkPortMapping[
               'lnk01'], "140.1.1.10", "255.255.255.0", "140.1.1.255", False))
        LogOutput('info', "Unconfiguring workstation 2")
        finalResult.append(configureWorkstation(wrkston02Obj, wrkston02Obj.linkPortMapping[
               'lnk10'], "140.1.1.11", "255.255.255.0", "140.1.1.255", False))
