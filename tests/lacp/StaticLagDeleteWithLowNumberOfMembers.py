...
def createLAG(deviceObj, lagId, configure, intArray, mode):
    if configure:
        ...
        if retStruct.returnCode() != 0:
            LogOutput('error',"Failed to create LAG" + str(lagId) + " on " + deviceObj.device)
            return False
        else:
        ...
        if retStruct.returnCode() != 0:
            LogOutput('error',"Failed to verify if LAG" + str(lagId) + " was created on " + deviceObj.device)
            return False
        ...
        if retStruct.returnCode() != 0:
            LogOutput('error',"Failed to delete LAG on " + deviceObj.device)
            return False
        else:
...
class Test_ft_framework_basics:
    ...
    def test_createLAGs(self):
        ...
        dut01Obj = self.topoObj.deviceObjGet(device="dut01")
        dut02Obj = self.topoObj.deviceObjGet(device="dut02")
        assert(createLAG(dut01Obj, '1', True, [dut01Obj.linkPortMapping['lnk02'], dut01Obj.linkPortMapping['lnk03']], 'off'))
        assert(createLAG(dut02Obj, '1', True, [dut02Obj.linkPortMapping['lnk02'], dut02Obj.linkPortMapping['lnk03']], 'off'))
    ...
    def test_pingBetweenClients2(self):
        ...
        wrkston01Obj = self.topoObj.deviceObjGet(device="wrkston01")
        wrkston02Obj = self.topoObj.deviceObjGet(device="wrkston02")
        assert(pingBetweenWorkstations(wrkston01Obj, wrkston02Obj, "140.1.1.11", False))
    ...
    def test_createLAG1Interface(self):
        ...
        dut01Obj = self.topoObj.deviceObjGet(device="dut01")
        dut02Obj = self.topoObj.deviceObjGet(device="dut02")
        assert(createLAG(dut01Obj, '1', True, [dut01Obj.linkPortMapping['lnk02']], 'off'))
        assert(createLAG(dut02Obj, '1', True, [dut02Obj.linkPortMapping['lnk02']], 'off'))
