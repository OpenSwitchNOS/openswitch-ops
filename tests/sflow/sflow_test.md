...
- [Changing agent interface IP](#changing-agent-interface-ip)
...
6. Configure the collector IP address (attached to interface 2 of the switch).
   Set the agent-interface (interface 3), polling interval at 15 seconds and set the sampling rate at 20 (One in 20 packets are sampled)
    ```
...
7. Ping between Host-1 and the switch (CPU destined traffic). The sflowtool is able to see FLOW packets as well as CNTR packets from the switch.
    ```
    ping 10.10.10.1 -c 100 -i 0.02
    ```

8. Ping between Host-1 and Host-2 (Layer-3 traffic). The sflowtool is able to see FLOW packets as well as CNTR packets from the switch.
    ```
    ping 10.10.11.2 -c 100 -i 0.02
    ```
...
10. Ping again and check if the sflowtool is still able to receive the sFlow packets from the switch.
...
13. Ping between the hosts (Layer-2 traffic). The sflowtool is able to see FLOW packets as well as CNTR packets from the switch.
    ```
    ping 10.10.11.2 -c 100 -i 0.02
    ```
...
5. Configure the IP address of all three sFlow collectors.
   Set the agent-interface (interface 3), polling interval at 15 seconds and the sampling rate at 20 (One in 20 packets are sampled)
    ```
...
6. Ping between the Host and the switch. All the sFlow collectors should be able to see the FLOW packets & the CNTR packets from the switch.
    ```
    ping 10.10.10.1 -c 100 -i 0.02
    ```
...
6. Set the sampling rate to 100 (One in 100 packets are sampled).
    ```
    ops-as5712(config)# sflow sampling 100
    ```

7. Ping 1000 packets between the Host and the switch. The sflowtool is able to see at least 8 FLOW packets (taking into account randomness) from the switch.
    ```
...
9. Ping 1000 packets between the Host and the switch. The sflowtool is able to see at least 3 FLOW packets (randomness) from the switch.
    ```
    ping 10.10.10.1 -c 1000 -i 0.01
    ```
...
## Changing agent interface IP
### Objective
This test verifies that the configured agent interface IP is used by the control and flow packets.
### Requirements
- Physical switch/workstations test setup
...
5. Configure the collector IP address (attached to interface 2 of the switch).
   Set the agent-interface (interface 1) and set the sampling rate at 20 (One in 20 packets are sampled)
    ```
    ops-as5712(config)# sflow collector 10.10.11.2
...
6. Ping between Host-1 and the switch (CPU destined traffic).
    ```
    ping 10.10.10.1 -c 100 -i 0.02
    ```

7. Ping between the hosts (Layer-2 traffic). The sflowtool is able to see FLOW packets as well as CNTR packets from the  switch which have the configured agent interface ip.

8. Reconfigure the ageint interface ip to a different value.
   ```
   ops-as5712(config)# sflow agent-interface 3
   ```
9. Ping between Host-1 and the switch (CPU destined traffic).
    ```
    ping 10.10.10.1 -c 100 -i 0.02
    ```

10. Ping between the hosts (Layer-2 traffic). The sflowtool is able to see FLOW packets as well as CNTR packets from the switch which have the new agent interface ip.

### Test result criteria
#### Test pass criteria
- When sflow is enabled the agent IP address in the FLOW and CNTR packets match the configured agent interface's IP address (interface 1 and 2  in the above configuration).
