...
### Objective
This test verifies the global sFlow enable or disable functionality on the switch.
### Requirements
...
1. Connect Host to the switch and configure an IP address.
```
...
```
3. Configure the interfaces on the switch that are connected to these two hosts.
   Also configure another interface that is used during the sFlow configuration.
```
...
5. Configure the collector's IP address (attached to interface 2 of the switch).
   Set the agent-interface (interface 3), polling interval at 15 seconds and set the sampling rate at 25 (One in 25 packets are sampled)
```
...
```
6. Ping between Host and the switch. The sflowtool is able to see FLOW packets as well as CNTR packets from the switch.
```
ping 10.10.10.1 -c 100 -i 0.01
```
7. Disable the global sFlow feature on the switch.
```
ops-as5712(config)# no sflow enable
```
8. Ping again and check if the sflowtool is still able to receive the sFlow packets from the switch.
### Test result criteria
#### Test pass criteria
- When sFlow is enabled, the sflowtool is able to see the sFlow packets that are sent from the switch. The agent IP address in the FLOW and CNTR packets match the configured agent interface's IP address (interface 3 in the above configuration).
...
5. Configure the IP address of all three sFlow collectors.
   Set the agent-interface (interface 3), polling interval at 15 seconds and the sampling rate at 25 (One in 25 packets are sampled)
```
ops-as5712(config)# sflow collector 10.10.11.2
...
ops-as5712(config)# sflow sampling 25
```
6. Try to add one more collector. It must throw an error as we can only configure a maximum of three collectors.
```
ops-as5712(config)# sflow collector 10.10.14.2
Error: Cannot configure more than 3 collectors.
```
7. Ping between the Host and the switch. All the sFlow collectors should be able to see the FLOW packets & the CNTR packets from the switch.
```
ping 10.10.10.1 -c 100 -i 0.01
```
### Test result criteria
...
#### Test fail criteria
- When sFlow is enabled, any or all of the sFlow collectors dont receive sFlow packets from the switch.
...
1. Connect Host-1 to the switch interface 1. Configure IP & IPv6 addresses and routes.
```
ip addr add 10.10.10.2/24 dev eth1
...
```
2. Connect Host-2 to the switch interface 2. Configure IP & IPv6 addresses and routes.
```
ip addr add 10.10.11.2/24 dev eth1
...
ip addr add 10.10.12.2/24 dev eth1
```
4. Configure the interfaces on the switch that are connected to the hosts and the sFlow collector.
```
ops-as5712# configure terminal
ops-as5712(config)# interface 1
ops-as5712(config-if)# ip address 10.10.10.1/24
...
ops-as5712(config)# sflow enable
```
6. Configure the IP address of the sFlow collector.
   Set the agent-interface (interface 3), polling interval at 15 seconds and sampling rate at 25 (one in 25 packets are sampled)
```
ops-as5712(config)# sflow collector 10.10.12.2
ops-as5712(config)# sflow agent-interface 3
ops-as5712(config)# sflow polling 15
...
```
12. Add a new Layer-2 VLAN on the switch. Configure the interfaces connected to the hosts to become a part of this VLAN in access mode.
```
...
ping6 2000::2 -c 100 -i 0.01
```
14. Configure the interfaces to trunk VLAN mode on the switch to check if tagged packets are also being sampled.
```
ops-as5712(config)# interface 1
ops-as5712(config-if)# no routing
...
```
17. Ping between the hosts. The sFlow collector is able to see these tagged packets as FLOW packets from the switch.
```
ping 100.10.10.3 -c 100 -i 0.01
```
### Test result criteria
#### Test pass criteria
- When sflow is enabled, all types of packets must be sampled and sent to the sFlow collector.
#### Test fail criteria
- When sFlow is enabled, any or all of the packet types is not seen at the sFlow collector.
...
```
6. Set the sampling rate at 100 (One in 100 packets are sampled).
```
ops-as5712(config)# sflow sampling 100
```
7. Ping 1000 packets between the Host and the switch. The sflowtool is able to see atleast 8 FLOW packets (taking into account randomness) from the switch.
```
ping 10.10.10.1 -c 1000 -i 0.01
```
8. Change the sampling rate to 250 (One in 250 packets are sampled).
```
ops-as5712(config)# sflow sampling 250
```
9. Ping 1000 packets between the Host and the switch. The sflowtool is able to see atleast 3 FLOW packets (randomness) from the switch.
```
ping 10.10.10.1 -c 1000 -i 0.01
```
10. Set the polling interval at 15 seconds. The sflowtool is able to see atleast 3 CNTR packets from the switch in 1 minute.
```
ops-as5712(config)# sflow polling 15
```
11. Change the polling interval to 10 seconds. The sflowtool is able to see atleast 5 CNTR packets from the switch in 1 minute.
```
ops-as5712(config)# sflow polling 10
```
12. Set the header size at 128 bytes and ping from the host to the switch. The sflowtool is able to see FLOW packets with header containing 128 bytes.
```
ops-as5712(config)# sflow header-size 128
```
13. Change the header size to 256 bytes and ping from the host to the switch. The sflowtool is able to see FLOW packets with header containing 256 bytes.
```
ops-as5712(config)# sflow header-size 256
```
14. Set the max datagram size at 500 bytes and ping from the host to the switch. The sflowtool is able to see sFlow datagrams of size 500 bytes.
```
ops-as5712(config)# sflow max-datagram-size 500
```
14. Change the max datagram size to 1000 bytes and ping from the host to the switch. The sflowtool is able to see sFlow datagrams of size 1000 bytes.
```
ops-as5712(config)# sflow max-datagram-size 1000
```
### Test result criteria
#### Test pass criteria
- The sFlow feature is able to react dynamically to changing configuration.
#### Test fail criteria
- The sFlow feature does not react to dynamic changes in sFlow configuration.
