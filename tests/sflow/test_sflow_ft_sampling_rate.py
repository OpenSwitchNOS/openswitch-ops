...
"""
OpenSwitch Test for sFlow sampling rate.
"""
...
def test_sflow_sampling_rate(topology, step):
    """
    ...
    # We expect at least 80% of packets to be sampled and seen by the collector
    expected_samples = int(2 * ping_count/sampling_rate * (80/100))
    ...
    # Checking packets for sampling_rate 100 and ping_count 1000
    assert len(result['packets']) >= expected_samples
    ...
    # Checking packets for sampling rate 25 and ping_count 250
    assert len(result['packets']) >= expected_samples
