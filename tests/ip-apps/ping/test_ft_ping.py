...
class pingTest(OpsVsiTest):
    ...
    def pingIPv4Target(self):
        ...
        #Ping from s2 to s1
        ping_target_ip_set = False
        cmdOut = s2.cmdCLI("ping 10.0.10.1")
        for curLine in cmdOut.split('\n'):
        ...
        #Ping with multiple parameters
        ping_target_ip_set = False
        cmdOut = s2.cmdCLI("ping 10.0.10.1 data-fill dee datagram-size 200"
        ...
        info('\n')
        if ping_target_ip_set is True:
            info('### Ping from s2 to s1 with multiple parameters '
                 'is successful ###\n')
        else:
            info('### Ping from s2 to s1 with multiple parameters '
                 'is unsuccessful ###\n')
    ...
    def pingIPv4TargetIpOption(self):
        ...
        #Ping from s1 to s2 with ip-option record-route
        cmdOut = s1.cmdCLI("ping 10.0.10.2 ip-option record-route")
        for curLine in cmdOut.split('\n'):
            if 'RR:' in curLine:
                ping_target_ip_set = True
        ...
        info('\n')
        if ping_target_ip_set is True:
            info('### Ping from s1 to s2 with ip-option '
                 'record-route is successful ###\n')
        else:
            info('### Ping from s1 to s2 with ip-option '
                 'record-route is unsuccessful ###\n')

        #Ping from s1 to s2 with ip-option include-timestamp
        ping_target_ip_set = False
        cmdOut = s1.cmdCLI("ping 10.0.10.2 ip-option include-timestamp")
        for curLine in cmdOut.split('\n'):
            if 'TS:' in curLine:
                ping_target_ip_set = True
        ...
        if ping_target_ip_set is True:
            info('### Ping from s1 to s2 with ip-option '
                 'include-timestamp is successful ###\n')
        else:
            info('### Ping from s1 to s2 with ip-option '
                 'include-timestamp is unsuccessful ###\n')

        #Ping from s2 to s1 with ip-option include-timestamp-and-address
        ping_target_ip_set = False
        cmdOut = s2.cmdCLI("ping 10.0.10.1 "
                           "ip-option include-timestamp-and-address")
        for curLine in cmdOut.split('\n'):
            if 'switch (10.0.10.2)' in curLine:
                ping_target_ip_set = True

        info('\n')
        if ping_target_ip_set is True:
            info('### Ping from s2 to s1 with ip-option '
                 'include-timestamp-and-address is successful ### \n')
        else:
            info('### Ping from s2 to s1 with ip-option '
                 'include-timestamp-and-address is unsuccessful ###\n')
    ...
    def pingIPv6Target(self):
        ...
        #Ping6 with multiple parameters
        ping_target_ipv6_set = False
