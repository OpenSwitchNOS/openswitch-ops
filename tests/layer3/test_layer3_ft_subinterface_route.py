...
#    hs2.libs.ip.add_route('1.1.1.0/24', '3.3.3.1')
    ...
    print("Check for route")
    sw1("ovs-appctl plugin/debug l3route", shell="bash")
    routes = sw1.libs.vtysh.show_ip_route()
    for route_entry in routes:
        if '3.3.3.0' == route_entry['id']:
            assert route_entry['prefix'] == '24', "Incorrect prefix"
            assert route_entry['next_hops'][0]['via'] == '2.2.2.1',\
                "Incorrect nexthop"

    print("Check for egress")
    sw2("ovs-appctl plugin/debug l3egress", shell="bash")
    routes = sw2.libs.vtysh.show_ip_route()
    for route_entry in routes:
        if '1.1.1.0' == route_entry['id']:
            assert route_entry['prefix'] == '24', "Incorrect prefix"
            assert route_entry['next_hops'][0]['via'] == '2.2.2.2',\
                "Incorrect nexthop"
