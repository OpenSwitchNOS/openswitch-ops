...
def l3_route(**kwargs):
    ...
    interface=wrkstn2.linkPortMapping['lnk02'], config=True)
    ...
    netMask="255.255.255.0", broadcast="192.168.1.255",
    interface=wrkstn3.linkPortMapping['lnk03'], config=True)
    if retStructObj.returnCode() != 0:
        LogOutput('error', "Failed to configure IP on workstation")
    ...
    LogOutput('info', "ping workstation 1 to workstation 2 return JSON:\n" + str(retStruct.retValueString()))
...
def test_2(**kwargs):
    ...
    retStruct = Dot1qEncapsulation(deviceObj=device1,subInterface="1.10", dot1q=True, vlan=10, enable=True)
    # configure Ip on the host
    retStructObj = wrkstn1.NetworkConfig(ipAddr="192.168.1.2",
    netMask="255.255.255.0", broadcast="192.168.1.255",
    interface=wrkstn1.linkPortMapping['lnk01'], config=True)
...
class Test_subInt:
    ...
    def test_l3_route(self):
        ...
        retValue = l3_route(device1=dut01Obj, device2=dut02Obj, device3=wrkston1Obj, device4=wrkston2Obj)
