...
def l3_route(**kwargs):
    ...
    retStruct = Dot1qEncapsulation(deviceObj=device1,subInterface="1.10", dot1q=True, vlan=10)
    if retStruct.returnCode() != 0:
        LogOutput('error',"Failed to enable interface")
        assert(False)
    ...
    retStructObj = wrkstn2.NetworkConfig(ipAddr="192.168.1.2",
		        ...
		        interface=wrkstn2.linkPortMapping['lnk02'],
		        config=True)
    ...
    retStructObj = wrkstn3.NetworkConfig(ipAddr="192.168.1.3",
				netMask="255.255.255.0",
				broadcast="192.168.1.255",
		        interface=wrkstn3.linkPortMapping['lnk03'],
		        config=True)
    if retStructObj.returnCode() != 0:
        LogOutput('error', "Failed to configure IP on workstation")
...
def test_2(**kwargs):
    ...
    retStructObj = wrkstn1.NetworkConfig(ipAddr="192.168.1.2",
				netMask="255.255.255.0",
				broadcast="192.168.1.255",
		        interface=wrkstn1.linkPortMapping['lnk01'],
		        config=True)
    if retStructObj.returnCode() != 0:
        LogOutput('error', "Failed to configure IP on workstation")
