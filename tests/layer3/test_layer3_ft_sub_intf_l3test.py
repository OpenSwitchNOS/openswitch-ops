...
# pinging with ipv6 address
    ...
    retStruct = Dot1qEncapsulation(deviceObj=device1, subInterface="1.2",
                                   dot1q=True, vlan=12)
    retCmdout = wrkstn1.DeviceInteract(command="ip link add link wrkston01-eth0 name host01-eth0.12 type vlan id 12")
    retCmdout = wrkstn1.DeviceInteract(command="ip link set dev host01-eth0.12 up")
    retCmdout = wrkstn1.DeviceInteract(command="ip -6 address add 2000::9/64 dev host01-eth0.12")
