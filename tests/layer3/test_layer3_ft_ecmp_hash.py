...
#Note: There is a possibility that the tuples this script uses would not pick
#different nexthops when ECMP configuration changes in the future. Should use
#the ASIC APIs to pick the right tuples programatically.
...
# a server to be configured and running on another host.
...
class Test_ft_ecmp_hash:
    ...
    def test_l4src_hash(self):
        ...
        disable_all_ecmp_hash(dut01Obj)
        enable_ecmp_hash(dut01Obj, "src-port")
        intf1, intf2 = run_nexthop_calc(dut01Obj, intf1sw1, intf2sw1,
                                        10024, "30.0.0.1", 10024,
                                        10023, "30.0.0.1", 10024)

        if intf1 == intf2:
            LogOutput('error', "src port hashing did not work")
            error(False)
        else:
    ...
    def test_l4dst_hash(self):
        LogOutput('info', "\n###############################################")
        LogOutput('info', "# Step 4 - Test dest port hash")
        LogOutput('info', "################################################")

        disable_all_ecmp_hash(dut01Obj)
        enable_ecmp_hash(dut01Obj, "dst-port")
        intf1, intf2 = run_nexthop_calc(dut01Obj, intf1sw1, intf2sw1,
                                        10024, "30.0.0.1", 10024,
                                        10024, "30.0.0.1", 10023)

        if intf1 == intf2:
            LogOutput('error', "dst port hashing did not work")
            error(False)
        else:
    ...
    def test_l3dst_hash(self):
        ...
        disable_all_ecmp_hash(dut01Obj)
        enable_ecmp_hash(dut01Obj, "dst-ip")
        intf1, intf2 = run_nexthop_calc(dut01Obj, intf1sw1, intf2sw1,
                                        10024, "30.0.0.1", 10024,
                                        10024, "30.0.0.4", 10024)

        if intf1 == intf2:
            LogOutput('error', "dst IP hashing did not work")
            error(False)
        else:
    ...
    def test_l3src_hash(self):
        ...
        disable_all_ecmp_hash(dut01Obj)
        enable_ecmp_hash(dut01Obj, "src-ip")
        ...
        if intf1 == intf2:
            LogOutput('error', "src IP hashing did not work")
            error(False)
        else:
