...
#
##########################################################################
# Description: Verify a switch port ability to handle untagged frames
...
#                            if necessary
#
#                    FAILED -> Vlan is forwarding traffic unexpected
#
###########################################################################
...
def portsAssign(dut, ports, vlans):
    for port, vlan in zip(ports, vlans):
        returnStr = AddPortToVlan(
            ...
            interface=port, access=False,
            allowed=True, tag=True, config=True)
...
def workStationConfig(wrk1, wrk2, pConfig=True):
...
def cleanUp(dut, wrk1, wrk2):
    ...
    assert(workStationConfig(wrk1, wrk2, False))
...
class Test_ft_vlan_untagged_frames_trunk_port:

    def setup_class(cls):
        # Create Topology object and connect to devices
        Test_ft_vlan_untagged_frames_trunk_port.testObj = \
            testEnviron(
                topoDict=topoDict)
        Test_ft_vlan_untagged_frames_trunk_port.topoObj = \
            Test_ft_vlan_untagged_frames_trunk_port.testObj.topoObjGet()
        Test_ft_vlan_untagged_frames_trunk_port.dut01Obj = \
            Test_ft_vlan_untagged_frames_trunk_port.topoObj.deviceObjGet(
                device="dut01")
        Test_ft_vlan_untagged_frames_trunk_port.wrkston01 = \
            Test_ft_vlan_untagged_frames_trunk_port.topoObj.deviceObjGet(
                device="wrkston01")
        Test_ft_vlan_untagged_frames_trunk_port.wrkston02 = \
            Test_ft_vlan_untagged_frames_trunk_port.topoObj.deviceObjGet(
                device="wrkston02")
    ...
    def teardown_class(cls):
        ...
        Test_ft_vlan_untagged_frames_trunk_port.topoObj.terminate_nodes()
    ...
    def test_configureWorkStons(self):
        ...
        LogOutput(
            'info', "##################################################")
        assert(
            workStationConfig(self.wrkston01, self.wrkston02))

    def test_sendTraffic(self):
        LogOutput('info', "\n############################################")
        LogOutput(
            'info', "Step 8- Send signal and verify is not forwarded")
        LogOutput('info', "############################################")
        assert(verifyPing(self.wrkston01, '10.1.1.6'))
        assert(verifyPing(self.wrkston02, '10.1.1.5'))
