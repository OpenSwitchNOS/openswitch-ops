...
#
##########################################################################
# Description: Verify the functionality of deleting a VLAN with no ports
#              configured within a VLAN. The Vlan will be deleted from the
#              middle of the VLAN list or the VLAN not with
#              the highest or lowest VID.
#
...
class Test_vlan_state_removed_from_middle_of_table:
    ...
    def test_sendTraffic(self):
        ...
        self.wrkston01.NetworkConfig(
            interface="eth1", ipAddr="10.1.1.5", netMask="255.255.255.0",
            broadcast="10.1.1.255", config=True)
        self.wrkston02.NetworkConfig(
            interface="eth1", ipAddr="10.1.1.6", netMask="255.255.255.0",
            broadcast="10.1.1.255", config=True)
        self.wrkston03.NetworkConfig(
            interface="eth1", ipAddr="10.1.1.7", netMask="255.255.255.0",
            broadcast="10.1.1.255", config=True)
        if capture(deviceObj=self.wrkston01, interface="eth0",
                   number='1') != 0:
