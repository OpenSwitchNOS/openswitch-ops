...
# Name:        VlanName.py
#
# Description: Verify that a VLAN description can be added
#             to the switch and it can be modified correctly.
#
# Author:      Roy Ibarra
#
# Topology:      |Switch|
#
# Success Criteria:  PASS -> Vlan description is added and changed
#                            successfully.
#
#                    FAILED -> Unable to add or modify vlan
#                              description.
#
##########################################################################
...
def showVlanDescription(**kwargs):
    ...
    if retCode != 0:
        LogOutput('error', "Failed to execute command" + command)
    else:
        LogOutput('debug', "Command executed successfully" + command)
...
class Test_VlanNameTC:
    ...
    def test_vlan_description_length(self):
        LogOutput('info', "############################################")
        LogOutput('info', "Step 5-  Adding a string of 250 characters " +
        ...
        returnCLS = VlanDescription(deviceObj=self.dut01Obj, vlanId=30,
                                    ...
                                    + "_names_of_existing_vlans_in_the"
                                    + "_system__________________________"
        ...
        if result == 0:
            ...
            if myDict['description'] != "Verify_that_a_VLAN_string_"\
                + "name_has_a_limit_to_the_amount_of_characters_it_" +\
                    ...
                    "duplicate_names_of_existing_vlans_in_the_system" +\
                    "_______________________________________________" +\
                    "_______________________________________":
                LogOutput('error', "step failed. Vlan description was \
                not updated")
                assert 1 != 1
            else:
                LogOutput('info', "Passed - Vlan with id 30 accepted a \
                    description with more than 250 characters")
        else:
            LogOutput('error', "step failed, the DUT did not accepted a \
             description of more than 250 characters")
            assert result == 0
