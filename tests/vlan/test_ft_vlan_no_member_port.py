...
#            of them. Verify the state is down and no_member_port
#            in the reason value for one of them.
#
# Author:      Roy Ibarra
#
# Topology:      |Switch|
            ...
            for myDictionary in showVlanOutput:
                ...
                if myDictionary['VLAN'] == "40" and \
                        (myDictionary['Status'] != "down" or
                         myDictionary['Reason'] != "admin_down"):
                    LogOutput(
                        'error', "Step failed. Vlan 40 status or " +
                        "reason value is other than down and admin_down")
                    assert 1 != 1
                if myDictionary['VLAN'] == "50" and \
                        (myDictionary['Status'] != "down" or
                         myDictionary['Reason'] != "no_member_port"):
                    LogOutput(
                        'error', "Step failed. Vlan 50 status or reason" +
                        " value is other than down and no_member_port")
                    assert 1 != 1
