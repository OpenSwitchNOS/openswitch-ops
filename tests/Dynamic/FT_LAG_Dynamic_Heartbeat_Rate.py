...
def test_lacpd_heartbeat(topology):
    ...
    for curr_ops in [ops1, ops2]:
        ...
        for curr_p in lag_interfaces + [host_interface]:
            ...
            assert not curr_ops(
                ...
                format(**locals()),
                shell='vsctl'
            ), 'Failed to set interface {px} up'.format(**locals())
    ...
    for lag_rate_mode in ['slow', 'fast']:
        ...
        for c_sw in [ops1, ops2]:
            switch_config_lacp_rate(c_sw, test_lag, lag_rate_mode)
        ...
        for curr_ops in [ops1, ops2]:
            ...
            print('Wait for tcpdump finished capturing packets')
            sleep(wait_time)
