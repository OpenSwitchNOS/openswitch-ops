...
"""
OpenSwitch Test for converting static lag to dynamic lag
...
def create_static_lag(ops, lag_id, step):
...
def static_lag_to_dynamic_lag(ops, lag_id, mode, step):
    step("converting static lag to dynamic lag")
    with ops.libs.vtysh.ConfigInterfaceLag(lag_id) as ctx:
        if mode == "active":
...
def test_lag_ft_staticlagconverttodynamic(topology, step):
    ...
    # Create static LAG :
    create_static_lag(ops1, lag_id, step)
    create_static_lag(ops2, lag_id, step)
    ...
    static_lag_to_dynamic_lag(ops1, lag_id, "active",  step)
    static_lag_to_dynamic_lag(ops2, lag_id, "passive", step)
    sleep(10)
